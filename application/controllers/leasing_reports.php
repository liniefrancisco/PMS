...
class Leasing_reports extends CI_Controller
{
    ...
    public function generate_paymentReport()
    {
        if ($this->session->userdata('leasing_logged_in')) {
            ...
            foreach ($report as $key => $r) {
                ...
                $reportData[$key]['payment_date'] = $r['payment_date'];
                $reportData[$key]['payor'] = $r['payor'];
                $reportData[$key]['tin'] = $r['tin'];
                $reportData[$key]['tender_typeDesc'] = $r['tender_typeDesc'];
                $reportData[$key]['receipt_no'] = $r['receipt_no'];

                $slData = $this->app_model->findInSubsidiaryLedger($r['receipt_no']);

                if (!empty($slData)) {
                    foreach ($slData as $sld) {
                        if ($sld['gl_accountID'] == '4') {
                            $rental += $sld['credit'];

                            if ($sld['document_type'] == 'Payment Adjustment') {
                                if ($sld['credit'] !== '' && $sld['credit'] !== null) {
                                    $rental += $sld['credit'];
                                } else {
                                    $rental += $sld['debit'];
                                }
                            }
                        } elseif ($sld['gl_accountID'] == '22') {
                            $other += $sld['credit'];
                        ...
                        } elseif ($sld['gl_accountID'] == '7') {
                            $advance += str_replace('-', '', $sld['credit']);
                        ...
                        } elseif (in_array($sld['gl_accountID'], ['8', '9'])) {
                            $preop += str_replace('-', '', $sld['credit']);
                        }
                    }
                }

                $reportData[$key]['preop'] = $preop;
                $reportData[$key]['advance'] = $advance;
                $reportData[$key]['basic'] = abs($rental);
                $reportData[$key]['other'] = abs($other);
                $reportData[$key]['total'] = abs($preop + $rental + $other);
                $reportData[$key]['payment'] = $r['amount_paid'];
            }

            if ($doc_type == 'excel') {
                return $this->paymentReportExcel($reportData, $file_name);
            }
            ...
            $pdf->setFont('times', '', 10);
            $pdf->cell(1, 4, 'Date Printed: ' . date('F j, Y h:i:s:A'), 0, 0, 'L');
            $pdf->ln();
            $pdf->cell(0, 4, 'Transaction Date: ' . date('m/d/y', strtotime($from_date)) . ' To ' . date('m/d/y', strtotime($to_date)), 0, 0, 'L');
            $pdf->ln();
            $pdf->cell(0, 4, 'Generated By: ' . $cashier_name, 0, 0, 'L');
            $pdf->ln();
            $pdf->setFont('times', 'BU', 10);
            $pdf->cell(335, 5, '', 'B', 0, 'L');
            $pdf->ln();
            $pdf->setFont('times', 'B', 14);
            $pdf->cell(0, 10, 'Payment Report List', 0, 0, 'C');
            $pdf->ln();
            $pdf->setFont('times', 'BU', 14);
            $pdf->cell(335, 5, '', 'T', 0, 'L');
            $pdf->ln();

            // =================== Payment Table ============= //
            $pdf->setFont('times', 'B', 9);
            $pdf->cell(20, 8, 'Payment Date', 0, 0, 'L');
            $pdf->cell(85, 8, 'Payor', 0, 0, 'C');
            $pdf->cell(33.5, 8, 'TIN', 0, 0, 'C');
            $pdf->cell(33.5, 8, 'Tender Type', 0, 0, 'C');
            $pdf->cell(33.5, 8, 'OR #', 0, 0, 'C');
            $pdf->cell(26, 8, 'Preop', 0, 0, 'R');
            $pdf->cell(26, 8, 'Advance', 0, 0, 'R');
            $pdf->cell(26, 8, 'Rental', 0, 0, 'R');
            $pdf->cell(26, 8, 'Others', 0, 0, 'R');
            $pdf->cell(26, 8, 'Amount Paid', 0, 0, 'R');
            ...
            foreach ($reportData as $value) {
                // dump($value);
                $pdf->ln();
                $pdf->cell(20, 8, $value['payment_date'], 1, 0, 'L');
                $pdf->cell(85, 8, $value['payor'], 1, 0, 'L');
                $pdf->cell(33.5, 8, $value['tin'], 1, 0, 'L');
                $pdf->cell(33.5, 8, $value['tender_typeDesc'], 1, 0, 'L');
                $pdf->cell(33.5, 8, $value['receipt_no'], 1, 0, 'L');
                $pdf->cell(26, 8, number_format($value['preop'], 2), 1, 0, 'R');
                $pdf->cell(26, 8, number_format($value['advance'], 2), 1, 0, 'R');
                $pdf->cell(26, 8, number_format($value['basic'], 2), 1, 0, 'R');
                $pdf->cell(26, 8, number_format($value['other'], 2), 1, 0, 'R');
                $pdf->cell(26, 8, number_format($value['payment'], 2), 1, 0, 'R');

                $total += $value['payment'];
                $total_preop += $value['preop'];
                $total_advance += $value['advance'];
                $total_basic += $value['basic'];
                $total_other += $value['other'];
            }

            $pdf->ln();
            $pdf->cell(335, 8, '', 'B', 0, 'L');
            $pdf->ln();
            $pdf->setFont('times', 'B', 9);
            $pdf->cell(205.5, 8, '', 0, 0, 'L');
            $pdf->cell(26, 8, 'Preop', 0, 0, 'R');
            $pdf->cell(26, 8, 'Advance', 0, 0, 'R');
            $pdf->cell(26, 8, 'Rental', 0, 0, 'R');
            $pdf->cell(26, 8, 'Others', 0, 0, 'R');
            ...
            $pdf->ln();
            $pdf->setFont('times', 'B', 9);
            $pdf->cell(205.5, 8, 'Total Amount ', 0, 0, 'L');

            $pdf->setFont('times', 'BUU', 9);
            $pdf->cell(26, 8, number_format($total_preop, 2), 0, 0, 'R');
            $pdf->cell(26, 8, number_format($total_advance, 2), 0, 0, 'R');
            $pdf->cell(26, 8, number_format($total_basic, 2), 0, 0, 'R');
            $pdf->cell(26, 8, number_format($total_other, 2), 0, 0, 'R');
            $pdf->cell(26, 8, number_format($total, 2), 0, 0, 'R');

            $this->db->trans_complete(); // End of transaction function

            if ($this->db->trans_status() === false) {
                // Check if the function is failed or succeed
                $this->db->trans_rollback(); // If failed rollback all queries
                $error = [
                    'action' => 'Saving Payment',
                    'error_msg' => $this->db->_error_message(),
                ]; //Log error message to `error_log` table
                $this->app_model->insert('error_log', $error);
                $response['msg'] = 'DB_error';
            } else {
                $response['file_name'] =
                    base_url() . 'assets/pdf/' . $file_name;
                $pdf->Output('assets/pdf/' . $file_name, 'F');
                $response['msg'] = 'Success';
            }
    ...
    function paymentReportExcel($report, $file_name)
    {
        ...
        ($file = fopen($filedir, 'w')) or
            die(json_encode(['msg' => 'Cannot create file']));
    ...
    public function get_orNumberEntry()
    {
        if ($this->session->userdata('leasing_logged_in') || $this->session->userdata('cfs_logged_in')) {
            $tenant_id = $this->uri->segment(3);
            $result = $this->app_model->get_paymentScheme2($tenant_id);
            echo json_encode($result);
        }
    }
    ...
    public function generate_accountabilityReport()
    {
        if ($this->session->userdata('leasing_logged_in') && $this->session->userdata('user_type') == 'Administrator') {
            ...
            $cash_secdepo_amount = 0;
            $cash_consbond_amount = 0;
            $check_rr_amount = 0;
            $check_ar_amount = 0;
            $check_ri_amount = 0;
            $check_vat_amount = 0;
            $check_wht_amount = 0;
            $check_advance_amount = 0;
            $check_secdepo_amount = 0;
            $check_consbond_amount = 0;
            $cash_total = 0;
            $check_total = 0;
            $grand_total = 0;
    ...
    // ------------------------
    public function monthly_receivable_summary_new()
    {
        if ($this->session->userdata('leasing_logged_in')) {
            $data['flashdata'] = $this->session->flashdata('message');
            $data['expiry_tenants'] = $this->app_model->get_expiryTenants();
    ...
    //         var_dump('reportData:',$reportData);
    //         $reportDataNew = [];
    ...
    //             $reportDataNew[$value['tenant_id']]['tenant_id'] = $value['tenant_id'];
    //             $reportDataNew[$value['tenant_id']]['trade_name'] = $value['trade_name'];
    //             //$reportDataNew[$value['tenant_id']]['tin'] = $value['tin']; // Added by gwapss
    //             $reportDataNew[$value['tenant_id']]['total'] = 0;

    //             if($value['gl_accountID'] === '4'){
    //                 $reportDataNew[$value['tenant_id']]['basic'] = $value['amount'];
    //                 $reportDataNew[$value['tenant_id']]['total'] += $value['amount'];

    //             }else{
    //                 $reportDataNew[$value['tenant_id']]['others'] = $value['amount'];
    //                 $reportDataNew[$value['tenant_id']]['total'] += $value['amount'];

    //             }
    //         }

    //         // $reportDataBasic = $this->app_model->generate_monthly_receivable_summary_basic($month);
    //         // $reportDataOther = $this->app_model->generate_monthly_receivable_summary_other($month);
    ...
    //         $this->excel->generate_monthly_receivable_summary($reportDataNew, $ar_total, $rr_total, $filename, $month);
    //     } else {
    //         redirect('ctrl_leasing/');
    //     }
    // }
    ...
    { //LINIE
        if ($this->session->userdata('leasing_logged_in')) {
            ...
            foreach ($reportData as $key => $value) {
                ...
                $reportDataNew[$value['tenant_id']]['basic'] = $value['basic_amount'];
                $reportDataNew[$value['tenant_id']]['others'] = $value['other_amount'];
                $reportDataNew[$value['tenant_id']]['amount'] += $value['amount'];
    ...
    // --------------------------------------------
    public function generate_monthly_receivable_summary_new()
    {
        if ($this->session->userdata('leasing_logged_in')) {
            $month = $this->sanitize($this->input->post('month'));
            $storecode = $this->session->userdata('store_code');
            $reportData = $this->app_model->generate_monthly_receivable_summary_new($month);
            $reportDataNew = [];
            ...
            $this->excel->generate_monthly_receivable_summary_new($reportDataNew, $filename, $month);
        } else {
            redirect('ctrl_leasing/');
        }
    }
    ...
    //             // Initialize tenant data if not already set
    //             if (!isset($reportDataNew[$tenantID])) {
    //                 $reportDataNew[$tenantID] = [
    //                     'tenant_id' => $tenantID,
    //                     'trade_name' => $value['trade_name'],
    //                     'total' => 0,
    //                     'basic' => 0,  // Initialize basic to 0
    //                     'others' => 0, // Initialize others to 0
    //                 ];
    //             }

    //             // Add to 'basic' or 'others' based on gl_accountID
    //             if ($value['gl_accountID'] === '4') {
    //                 $reportDataNew[$tenantID]['basic'] += $value['amount'];  // Use outstanding_balance

    //             } else {
    //                 $reportDataNew[$tenantID]['others'] += $value['amount'];  // Use outstanding_balance
    //             }

    //             // Update the total outstanding balance for the tenant
    //             $reportDataNew[$tenantID]['total'] += $value['amount'];
    //         }
    ...
    public function AR_reports()
    {
        if ($this->session->userdata('leasing_logged_in')) {
            ...
            $toExport = $this->db->query("SELECT sl.doc_no, sl.posting_date
                                      FROM subsidiary_ledger sl
                                      WHERE (sl.document_type = 'Invoice')
                                      ...
                                      AND DATE_FORMAT(sl.posting_date, '%Y') = '" . date('Y') . "'
                                      ...
                                      AND sl.tenant_id LIKE '%{$this->session->userdata('store_code')}%'
                                      GROUP BY sl.doc_no, sl.tenant_id")->result_array();
            ...
            $toExport2 = $this->db->query("SELECT doc_no, posting_date
                                            ...
                                            GROUP BY doc_no")->result_array();
            if (isset($toExport2)) {
                foreach ($toExport2 as $key => $value) {
                    $date = date('m', strtotime($value['posting_date']));
                    switch ($date) {
                        case '01':
                            $data['JAN'] += 1;
        ...
        if (!empty($data)) {
            ...
            foreach ($data as $result) {
                ...
                if ($result['gl_code'] == '10.10.01.03.03' || $result['gl_code'] == '10.10.01.03.04') {
                    ...
                    $rows[] = array("GENERAL<|>{$line_no}<|>G/L Account<|>{$result['gl_code']}<|>{$posting_date}<|><|>{$docno}<|>{$result['gl_account']}<|>{$result['amount']}<|>{$company_code}<|>{$dept_code}<|>GENJNL<|>LEASING<|><|><|><|>{$result['doc_no']}<|><|>");
                } else {
                    $rows[] = array("GENERAL<|>{$line_no}<|>G/L Account<|>{$result['gl_code']}<|>{$posting_date}<|><|>{$docno}<|>{$result['gl_account']}<|>{$result['amount']}<|>{$company_code}<|>{$dept_code}<|>GENJNL<|>LEASING<|><|><|><|>{$result['doc_no']}<|><|>");
                }
                ...
                $file_name = "Other Charges $filter_date - " . $exp_batch_no . ".pjn";
    ...
    public function generate_RRreports()
    {
        ...
        $month = $this->sanitize($this->input->post('month'));
        // $type         = $this->sanitize($this->input->post('file_type'));
        $month = date('F Y', strtotime($month));
        $posting_date = date('m/d/Y', strtotime(date('Y-m-t', strtotime($month))));
        $company_code = $this->session->userdata('company_code');
        $dept_code = $this->session->userdata('dept_code');
        ...
        if (!empty($data)) {
            ...
            foreach ($data as $result) {
                ...
                if ($result['gl_code'] == '10.10.01.03.16') {
                    $rows[] = array("GENERAL<|>{$line_no}<|>Customer<|>{$result['tenant_id']}<|>{$pDate}<|><|>{$doc_no}<|>{$result['trade_name']}<|>{$result['amount']}<|>{$company_code}<|>{$dept_code}<|>GENJNL<|>LEASING<|><|><|><|>{$result['doc_no']}<|><|>");
                } else if ($result['gl_code'] == '10.10.01.06.05') {
                    $rows[] = array("GENERAL<|>{$line_no}<|>G/L Account<|>{$result['gl_code']}<|>{$pDate}<|><|>{$doc_no}<|>{$result['gl_account']}<|>{$result['amount']}<|>{$company_code}<|>{$dept_code}<|>GENJNL<|>LEASING<|><|><|><|>{$result['doc_no']}<|><|>");
                } else {
                    $amount = $result['amount'];

                    $rows[] = array("GENERAL<|>{$line_no}<|>G/L Account<|>{$result['gl_code']}<|>{$pDate}<|><|>{$doc_no}<|>{$result['gl_account']}<|>{$amount}<|>{$company_code}<|>{$dept_code}<|>GENJNL<|>LEASING<|><|><|><|>{$result['doc_no']}<|><|>");
                }
            ...
            $file_name = "Basic Rent $filter_date - " . $exp_batch_no . ".pjn";
    ...
    public function generate_paymentCollection()
    {
        ...
        $file_name = "Payment Collection $post_date.pjn";
        $data = '';
        ...
        if (!empty($entries)) {
            ...
            foreach ($entries as $key => $entry) {
                ...
                if ($entry->gl_accountID == 4 || $entry->gl_accountID == 22 || $entry->gl_accountID == 29) {
                    ...
                    $data_csv[] = array("GENERAL<|>$line_no<|>Customer<|>$entry->tenant_id<|>$pDate<|>$docType<|>$doc_no<|>$entry->trade_name<|>$entry->amount<|>$entry->company_code<|>$entry->department_code<|>GENJNL<|>POS SALES<|><|><|><|>$entry->doc_no<|><|>");
                } else {
                    ...
                    $docType = $entry->document_type == 'Payment Adjustment' ? '' : 'Payment';
                    $data_csv[] = array("GENERAL<|>$line_no<|>Bank Account<|>$entry->bank_code<|>$pDate<|>$docType<|>$doc_no<|>$entry->bank_name<|>$entry->amount<|>$entry->company_code<|>$entry->department_code<|>GENJNL<|>POS SALES<|><|><|><|>$entry->doc_no<|><|>");
                }
            ...
            $file_name = "Payment Collection $post_date - " . $exp_batch_no . ".pjn";
            ...
            foreach ($reportData as $result) {
                ...
                if ($result->gl_accountID == 3) {
                    ...
                    $rows[] = array(
                        ...
                        $result->document_type == 'Payment Adjustment' ? '' : 'Payment',
    ...
    public function collection_reports()
    {
        if ($this->session->userdata('leasing_logged_in')) {
            ...
            $toExport = $this->app_model->generate_paymentCollection(date('Y'), "AND g.tenant_id LIKE '%{$this->session->userdata('store_code')}%'");
    ...
    public function save_adj_print_log()
    {
        ...
        $date = new DateTime();
        $timeStamp = $date->getTimestamp();
        $printed_on = date('Y-m-d', $timeStamp);
        $response = array();
        ...
        $data =
            [
                ...
                'printed_date' => $printed_on
            ];

        if (!empty($data)) {
            $this->db->trans_start(); // Transaction function starts here!!!

            $this->app_model->insert('adjustment_print_log', $data);

            $print_count = $this->app_model->count_print_adj($document_name);
            $printData = $this->app_model->get_adjustmentsData1($document_name);

            $file_name = $tenant_id . $printData->adj_no . '.pdf';
            ...
            // echo $memoType . " - " . $doc_type . " - " . ABS($amount) . " - " . $count . " - " . $file_name;

            $store_code = $this->app_model->tenant_storeCode($tenant_id);
            $store_details = $this->app_model->store_details(trim($store_code));
            $prospect_id = $this->app_model->get_pID($tenant_id);
            $trade_name = $this->app_model->get_tenantName($prospect_id);
            ...
            if ($doc_type == 'Invoice Adjustment') {
                // ============================ PDF INVOICE ================================================================================ //

                // ==================== Receipt Header ================== //

                if ($printData->debit == '' || $printData->debit == null) {
                    $memoType = 'Credit Memo';
                    $amount = $printData->credit;
                } else {
                    $memoType = 'Debit Memo';
                    $amount = $printData->debit;
                }

                if ($printData->gl_accountID == '22' || $printData->gl_accountID == '29') {
                    $doc_type = 'Other Charges';
                } else {
                    $doc_type = 'Basic Rent';
                }
                ...
                $pdf->ln();
                $pdf->setFont('times', 'B', 10);
                $pdf->cell(0, 5, "Print No: " . $print_count, 0, 0, 'L');
                ...
                $pdf->setFont('times', 'B', 16);
                $pdf->cell(0, 6, $memoType, 0, 0, 'C'); // Memo Type 
                $pdf->ln();
                $pdf->ln();
                $pdf->ln();

                $pdf->setFont('times', 'B', 10);
                $pdf->cell(30, 4, "Document No.", 0, 0, 'L');
                $pdf->cell(10, 4, " : ", 0, 0, 'C');
                $pdf->setFont('times', '', 10);
                $pdf->cell(15, 4, $printData->adj_no, 0, 0, 'L');

                $pdf->cell(75, 4, " ", 0, 0, 'L');

                $pdf->setFont('times', 'B', 10);
                $pdf->cell(30, 4, "Date Adjusted", 0, 0, 'L');
                $pdf->cell(10, 4, " : ", 0, 0, 'C');
                $pdf->setFont('times', '', 10);
                $pdf->cell(15, 4, $printData->transaction_date, 0, 0, 'L');

                $pdf->ln();

                $pdf->setFont('times', 'B', 10);
                $pdf->cell(30, 4, "Tenant ID", 0, 0, 'L');
                $pdf->cell(10, 4, " : ", 0, 0, 'C');
                $pdf->setFont('times', '', 10);
                $pdf->cell(15, 4, $tenant_id, 0, 0, 'L');

                $pdf->cell(75, 4, " ", 0, 0, 'L');

                $pdf->setFont('times', 'B', 10);
                $pdf->cell(30, 4, "Posting Date", 0, 0, 'L');
                $pdf->cell(10, 4, " : ", 0, 0, 'C');
                $pdf->setFont('times', '', 10);
                $pdf->cell(15, 4, $printData->posting_date, 0, 0, 'L');

                $pdf->ln();
                $pdf->ln();

                $pdf->setFont('times', 'B', 10);
                $pdf->cell(30, 4, "Adjusted the account of", 0, 0, 'L');
                $pdf->cell(10, 4, " : ", 0, 0, 'C');
                $pdf->setFont('times', '', 10);
                $pdf->cell(15, 4, $trade_name, 0, 0, 'L');
                $pdf->setFont('times', '', 10);

                $pdf->ln();

                $pdf->setFont('times', 'B', 10);
                $pdf->cell(30, 4, "Amounting to ", 0, 0, 'L');
                $pdf->cell(10, 4, " : ", 0, 0, 'C');
                $pdf->setFont('times', 'B', 11);
                $pdf->cell(25, 4, DecimalToWord::convert(ABS(str_replace(",", "", $amount)), 'Pesos', 'Centavos') . " only " . " (" . DecimalToWord::$formatted . ")", 0, 0, 'L');
                $pdf->setFont('times', '', 10);
                ...
                $pdf->setFont('times', 'B', 10);
                $pdf->cell(30, 4, "Document Type", 0, 0, 'L');
                $pdf->cell(10, 4, " : ", 0, 0, 'C');
                $pdf->setFont('times', '', 10);
                $pdf->cell(15, 4, $doc_type, 0, 0, 'L');

                $pdf->ln();

                $pdf->setFont('times', 'B', 10);
                $pdf->cell(30, 4, "Invoice No.", 0, 0, 'L');
                $pdf->cell(10, 4, " : ", 0, 0, 'C');
                $pdf->setFont('times', '', 10);
                $pdf->cell(15, 4, $printData->doc_no, 0, 0, 'L');

                $pdf->ln();

                $pdf->setFont('times', 'B', 10);
                $pdf->cell(30, 4, "Reason For Adjusting", 0, 0, 'L');
                $pdf->cell(10, 4, " : ", 0, 0, 'C');
                $pdf->setFont('times', '', 10);
                $pdf->cell(25, 4, $printData->adj_reason, 0, 0, 'L');
                $pdf->setFont('times', '', 10);
                ...
                $pdf->cell(10, 6, " ", 0, 0, 'R');
                $pdf->setFont('times', '', 9);
                $pdf->cell(35, 6, "Doc No", 1, 0, 'C', TRUE);
                $pdf->cell(35, 6, "Description", 1, 0, 'C', TRUE);
                $pdf->cell(35, 6, "Amount", 1, 0, 'C', TRUE);
                $pdf->cell(35, 6, "Adjustment", 1, 0, 'C', TRUE);
                $pdf->cell(35, 6, "Net Amount", 1, 0, 'C', TRUE);
                ...
                $pdf->cell(10, 6, " ", 0, 0, 'R');
                $pdf->setFont('times', '', 9);
                $pdf->cell(35, 6, $printData->doc_no, 1, 0, 'C');
                $pdf->cell(35, 6, $doc_type, 1, 0, 'C');
                $pdf->cell(35, 6, "P " . number_format($printData->prev_amount, 2, '.', ','), 1, 0, 'C');
                $pdf->cell(35, 6, "P " . number_format($amount, 2, '.', ','), 1, 0, 'C');
                $pdf->cell(35, 6, "P " . number_format($printData->net_balance, 2, '.', ','), 1, 0, 'C');
                ...
                $prepared_by = $this->app_model->preparedBy($this->session->userdata('id'));
                $pdf->setFont('times', '', 10);
                // $pdf->cell(0, 4, "Prepared By: " . $this->session->userdata('first_name') . " " . $this->session->userdata('last_name') . "      Check By:______________________", 0, 0, 'L');
                $pdf->cell(0, 4, "Prepared By: " . $printData->modified_by . "      Check By:______________________", 0, 0, 'L');
                $pdf->ln();
                $pdf->ln();
                $pdf->ln();
                $pdf->setFont('times', 'B', 10);
                ...
                {
                    $this->db->trans_rollback(); // If failed rollback all queries
                    $error = array('action' => 'Saving Print No', 'error_msg' => $this->db->_error_message()); //Log error message to `error_log` table
                    $this->app_model->insert('error_log', $error);
                    $response['msg'] = 'DB_error';
                } else {
                    $response['msg'] = "Success";
    ...
    public function fetch_adj_supp_doc()
    {
        $adj_no = $this->input->post('adj_no');
        $query = $this->db->query("SELECT * FROM `adjsupportingdoc` WHERE adj_no = '$adj_no'");
    ...
    public function tenants_contract()
    {
        if ($this->session->userdata('leasing_logged_in')) {
            $storeCOde = $this->session->userdata('store_code');
            $ledger = $this->db->query("SELECT 
                                    s.tenant_id, 
                                    g.gl_account,
                                    s.document_type,
                                    s.posting_date 
                                    FROM subsidiary_ledger s 
                                    LEFT JOIN gl_accounts g ON s.gl_accountID = g.id
                                    WHERE export_batch_code IS NULL
                                    AND s.gl_accountID IN ('4', '22')
                                    AND s.document_type IN ('Payment', 'Invoice')
                                    AND s.tenant_id LIKE '%$storeCOde%'")->RESULT_ARRAY();
            $data = [
                'ledger' => $ledger,
                'flashdata' => $this->session->flashdata('message'),
                'expiry_tenants' => $this->app_model->get_expiryTenants(),
                'current_date' => $this->_currentDate

            ];


            $this->load->view('leasing/header', $data);
            $this->load->view('leasing/reports/tenant_contracts');
            $this->load->view('leasing/footer');
        } else {
            redirect('ctrl_leasing');
        }
    ...
    public function generate_excel_contracts()
    {
        $tenant_type = $this->sanitize($this->input->post('tenancy_type'));
        $store = $this->sanitize($this->input->post('store'));
        $start_date = $this->sanitize($this->input->post('start_date'));
        $end_date = $this->sanitize($this->input->post('end_date'));
        $contract_st = $this->sanitize($this->input->post('contract_status'));
        $onContracts = $this->app_model->getOnContracts($store, $tenant_type, $start_date, $end_date, $contract_st);
        ...
        $csv_data[] = [
            ...
            'Opening Date',
            'Expiry Date',
            'Contract Docs In System(Uplaoded to Leasing)',
            'Contract Status',
            'Tenant Status',
            'Flag'
        ];

        foreach ($onContracts as $d) {
            $d = (object) $d;

            if ($d->p_status == 'On Contract' && $d->flag == 'Pending') {
                $contractStats = 'Pending Contract';
            } else {
                $contractStats = $d->p_status;
            }
        ...
        $file_name = "Active Contracts - $store " . date('Ymd', strtotime($date_from)) . '-' . date('Ymd', strtotime($date_to)) . '.csv';

        download_send_headers($file_name, $csv_data);
    }

    public function tenantswithholding()
    {
        if ($this->session->userdata('leasing_logged_in')) {
            $data['flashdata'] = $this->session->flashdata('message');
            $data['expiry_tenants'] = $this->app_model->get_expiryTenants();
            ...
            $this->load->view('leasing/header', $data);
            $this->load->view('leasing/reports/tenants_wht');
            $this->load->view('leasing/footer');
        } else {
            redirect('ctrl_leasing');
        }
    ...
    public function gettenantwht()
    {
        $tenancytype = str_replace("%20", " ", $this->uri->segment(2));
        $store = $this->uri->segment(3);
        $gettenantwht = $this->app_model->gettenantswht($tenancytype, $store);

        echo json_encode($gettenantwht);
    }

    public function export_tenantwht()
    {
        if ($this->session->userdata('leasing_logged_in')) {
            $tenancytype = str_replace("%20", " ", $this->uri->segment(3));
            $store = $this->uri->segment(4);
            $gettenantwht = $this->app_model->gettenantswht($tenancytype, $store);

            $date = new DateTime();
            $timeStamp = $date->getTimestamp();
            $filename = "TENANT_WITH_WITHHOLDING_{$tenancytype}_{$store}_{$timeStamp}";

            $this->excel->to_exceltenantledger($gettenantwht, $filename);
        }
    }

    public function tenantswithtin()
    {
        if ($this->session->userdata('leasing_logged_in')) {
            $data['flashdata'] = $this->session->flashdata('message');
            $data['expiry_tenants'] = $this->app_model->get_expiryTenants();
            ...
            $this->load->view('leasing/header', $data);
            $this->load->view('leasing/reports/tenants_tin');
            $this->load->view('leasing/footer');
        } else {
            redirect('ctrl_leasing');
        }
    ...
    public function gettenanttin()
    {
        $tenancytype = str_replace("%20", " ", $this->uri->segment(2));
        $store = $this->uri->segment(3);
        $tin_status = str_replace("%20", " ", $this->uri->segment(4));
        $gettenantwht = $this->app_model->gettenantstin($tenancytype, $store, $tin_status);

        echo json_encode($gettenantwht);
    }

    public function export_tenanttin()
    {
        if ($this->session->userdata('leasing_logged_in')) {
            $tenancytype = str_replace("%20", " ", $this->uri->segment(3));
            $store = $this->uri->segment(4);
            $tin_status = str_replace("%20", " ", $this->uri->segment(5));
            $gettenantwht = $this->app_model->gettenantstin($tenancytype, $store, $tin_status);

            $date = new DateTime();
            $timeStamp = $date->getTimestamp();
            $filename = "TENANT_TIN_REPORT{$tenancytype}_{$store}_{$timeStamp}";

            $this->excel->to_exceltenantledger($gettenantwht, $filename);
        }
    }

    // public function generate_RRreports_internal()
    // {
    //     $company_code      = $this->session->userdata('company_code');
    //     $dept_code         = $this->session->userdata('dept_code');
    ...
    public function generate_RRreports_internal()
    {
        ...
        $doc_no = 'LS' . date('mdy', strtotime(date('Y-m-t', strtotime($month))));
        $report_data = $this->app_model->generate_RRreports_internal($month);
        $data = $report_data['data'];
        $doc_nos = $report_data['doc_nos'];
        $file_data = '';
        // output each row of the data
        ...
        if (!empty($data)) {
            ...
            $exp_batch_no = $this->app_model->generate_ExportNo_internal(true);

            //Tag Export
            foreach ($doc_nos as $doc_no) {
                // $this->app_model->updateEntryAsExported('subsidiary_ledger', ['export_batch_internal' => $exp_batch_no], "doc_no = '$doc_no'   AND (export_batch_internal IS NULL OR export_batch_internal = '')");
            }
            ...
            //$insert to export log
            $this->app_model->logExportForNav(
                $exp_batch_no,
                $filter_date,
                $file_data,
                'Basic Rent',
    ...
    public function generate_ARreports_internal()
    {
        if (!$this->session->userdata('leasing_logged_in')) {
            redirect('ctrl_leasing/');
        }
        ...
        $doc_no = 'LS' . date('mdy', strtotime(date('Y-m-t', strtotime($month))));
        $report_data = $this->app_model->generate_ARreports_internal($month);
    ...
    public function generate_paymentCollection_internal()
    {
        $store = $this->session->userdata('store_code');
        $post_date = $this->sanitize($this->input->post('month'));
        $post_date = date('Y-m', strtotime($post_date));
        $entries = $this->db->query("SELECT 
                                    ...
                                    FROM
                                        ...
                                        DATE(g.posting_date) LIKE '%$post_date%'
                                            ...
                                            AND g.doc_no <> 'DELETED')
                                            AND g.tenant_id <> 'ICM-LT000064'
                                            AND (g.export_batch_internal IS NULL
                                            OR g.export_batch_internal = '')
                                        GROUP BY 
                                            g.doc_no, g.ref_no, g.gl_accountID, g.tenant_id, g.posting_date/*, g.document_type*/
                                        HAVING 
                                            amt <> 0
                                        ORDER BY 
                                            g.document_type , g.doc_no) AS tbl
                                    LEFT JOIN
                                        (SELECT s.*, s.gl_accountID AS partnerID FROM subsidiary_ledger s
                                         WHERE s.debit IS NULL 
                                         AND s.credit IS NOT NULL
                                         AND (s.document_type = 'Payment' OR s.document_type = 'Payment Adjustment')
                                         GROUP BY s.doc_no, s.ref_no , s.credit) sl2 
                                    ON tbl.doc_no = sl2.doc_no
                                    AND tbl.ref_no = sl2.ref_no
                                    AND tbl.db <> 0
                                    AND tbl.db = ABS(sl2.credit)
        ...
        if (!empty($entries)) {
            ...
            foreach ($entries as $key => $entry) {
                ...
                $bcode = $this->db->query("SELECT * FROM payment_scheme WHERE receipt_no = '$entry->doc_no'")->ROW();
                if (!empty($bcode)) {
                    switch ($store) {
                        case 'AM':
                            $bank_code = 'B-015';
                        ...
                        case 'ASCT':
                            ...
                            break;
                        case 'ICM':
                            $bank_code = ($bcode->tender_typeDesc == 'Bank to Bank') ? 'B-012' : 'B-007';
                            break;
                        case 'ACT':
                            $bank_code = 'B-003';
                            break;
                ...
                if ($entry->gl_accountID == 3) {
                    $data_csv[] = [
                        ...
                        $entry->document_type == 'Payment Adjustment' ? '' : 'Payment',
                        strval($entry->doc_no),
                        $doc_no,
