...
class Ctrl_validation extends CI_Controller {
    ...
    {
        $store   = str_replace('%20', " ", $this->uri->segment(3));
        $slot_no = str_replace('%20', " ", $this->uri->segment(4));
        ...
        $result = $this->db->query("SELECT l.id FROM location_slot l
                                    LEFT JOIN floors f ON l.floor_id = f.id
                                    LEFT JOIN stores s ON f.store_id = s.id
                                    WHERE l.slot_no = '$slot_no'
                                    AND s.store_name LIKE '$store'");
        if ($result->num_rows()>0) {
            echo true;
        } else {
            echo false;
    ...
    {
        $store   = str_replace('%20', " ", $this->uri->segment(3));
        $slot    = explode("_", str_replace('%20', " ", $this->uri->segment(4)));
        $slot_no = $slot[0];
        $id      = $slot[1];
    ...
    public function verify_receiptNo()
    {
        ...
        $result = $this->db->query("SELECT `id` FROM `payment_scheme` WHERE ROUND(`amount_paid`) >= 1 AND (`receipt_no` = '$receipt_no' OR `receipt_no` = 'PR".$receipt_no."')");
