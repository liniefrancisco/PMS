...
class Leasing extends CI_Controller
{
    ...
    function createSoaFile($soa, $debit)
    {
        ...
        $pdf->cell(18, 4, 'Trade Name', 0, 0, 'L');
        $pdf->cell(2, 4, ':', 0, 0, 'L');
        $pdf->cell(60, 4, $tenant->trade_name, 0, 0, 'L');
        $pdf->cell(50, 4, ' ', 0, 0, 'L');
        $pdf->cell(25, 4, 'SOA No.', 0, 0, 'L');
        $pdf->cell(2, 4, ':', 0, 0, 'L');
        $pdf->cell(25, 4, $soa->soa_no, 0, 0, 'L');
        $pdf->ln();
        $pdf->cell(18, 4, 'Corp Name', 0, 0, 'L');
        $pdf->cell(2, 4, ':', 0, 0, 'L');
        $pdf->cell(60, 4, $tenant->corporate_name, 0, 0, 'L');
        $pdf->cell(50, 4, '  ', 0, 0, 'L');
        $pdf->cell(25, 4, 'Date of Transaction', 0, 0, 'L');
        $pdf->cell(2, 4, ':', 0, 0, 'L');
        $pdf->cell(25, 4, $soa->date_created, 0, 0, 'L');
        $pdf->ln();
        $pdf->cell(18, 4, 'Address', 0, 0, 'L');
        $pdf->cell(2, 4, ':', 0, 0, 'L');
        $pdf->cell(60, 4, $tenant->address, 0, 0, 'L');
        $pdf->cell(50, 4, '  ', 0, 0, 'L');
        $pdf->cell(25, 4, 'Billing Period', 0, 0, 'L');
        $pdf->cell(2, 4, ':', 0, 0, 'L');
        if ($soa->billing_period === 'UPON SIGNING OF NOTICE') {
            $pdf->SetFont('times', 'B', 7);
        } else {
            $pdf->SetFont('times', 'B', 8);
        }
        $pdf->cell(25, 4, $soa->billing_period, 0, 0, 'L');
        $pdf->ln();
        $pdf->SetFont('times', 'B', 8);
        $pdf->cell(18, 4, 'TIN', 0, 0, 'L');
        $pdf->cell(2, 4, ':', 0, 0, 'L');
                        ...
                        if (!empty($data)) {
                            ...
                            foreach ($data as $result) {
                                ...
                                if ($result['gl_code'] == '10.10.01.03.16') {
                                    ...
                                    $rows[] = ["GENERAL<|>$line_no<|>Customer<|>{$result['tenant_id']}<|>$pDate<|><|>{$doc_no}<|>{$tradeName}-Rent<|>{$result['amount']}<|>$company_code<|>$dept_code<|>GENJNL<|>LEASING<|><|><|><|>{$formattedBillingPeriod};{$dueDateFormatted}<|><|>"];
                                    ...
                                    $rows[] = ["GENERAL<|>$line_no<|>G/L Account<|>{$result['gl_code']}<|>$pDate<|><|>{$doc_no}<|>{$result['gl_account']}<|>{$result['amount']}<|>$company_code<|>$dept_code<|>GENJNL<|>LEASING<|><|><|><|>{$formattedBillingPeriod};{$dueDateFormatted}<|><|>"];
                                } else {
                                    ...
                                    $rows[] = ["GENERAL<|>$line_no<|>G/L Account<|>{$result['gl_code']}<|>$pDate<|><|>{$doc_no}<|>{$result['gl_account']}<|>($amount)<|>$company_code<|>$dept_code<|>GENJNL<|>LEASING<|><|><|><|>{$formattedBillingPeriod};{$dueDateFormatted}<|><|>"];
                                }
                    ...
                    if ($checkBalance->debit > 0) {
                        ...
                        if (!empty($data)) {
                            ...
                            foreach ($data as $result) {
                                ...
                                //     $rows[] = ["GENERAL<|>{$line_no}<|>Customer<|>{$tenantID}<|>{$posting_date}<|><|>{$doc_no}<|>{$tradeName}-Other Charges<|>{$result['amount']}<|>{$company_code}<|>{$dept_code}<|>GENJNL<|>LEASING<|><|><|><|>{$result['doc_no']}<|><|>"];
                                // } elseif ($result['gl_code'] == '10.10.01.06.05') {
                                //     $rows[] = ["GENERAL<|>{$line_no}<|>G/L Account<|>{$result['gl_code']}<|>{$posting_date}<|><|>{$doc_no}<|>{$result['gl_account']}<|>{$result['amount']}<|>{$company_code}<|>{$dept_code}<|>GENJNL<|>LEASING<|><|><|><|>{$result['doc_no']}<|><|>"];
                                // } else {
                                //     $amount = str_replace('-', '', $result['amount']);
                                //     $rows[] = ["GENERAL<|>{$line_no}<|>G/L Account<|>{$result['gl_code']}<|>{$posting_date}<|><|>{$doc_no}<|>{$result['gl_account']}<|>({$amount})<|>{$company_code}<|>{$dept_code}<|>GENJNL<|>LEASING<|><|><|><|>{$result['doc_no']}<|><|>"];
                                // }

                                if (in_array($result['gl_code'], ['10.10.01.03.03', '10.10.01.03.04'])) {
                                    $rows[] = ["GENERAL<|>{$line_no}<|>Customer<|>{$tenantID}<|>{$posting_date}<|><|>{$doc_no}<|>{$tradeName}-Other Charges<|>{$result['amount']}<|>{$company_code}<|>{$dept_code}<|>GENJNL<|>LEASING<|><|><|><|>{$formattedBillingPeriod};{$dueDateFormatted}<|><|>"];
                                } elseif ($result['gl_code'] == '10.10.01.06.05') {
                                    ...
                                    // $rows[] = ["GENERAL<|>{$line_no}<|>G/L Account<|>{$result['gl_code']}<|>{$posting_date}<|><|>{$doc_no}<|>{$result['gl_account']}<|>{$result['amount']}<|>{$company_code}<|>{$dept_code}<|>GENJNL<|>LEASING<|><|><|><|>{$soa->billing_period}<|><|>",];
                                    // dump($invoicing);
                                    ...
                                    switch ($invoicing->charges_code) {
                                        case 'PC000007':
                                            ...
                                            $rows[] = ["GENERAL<|>{$line_no}<|>G/L Account<|>{$result['gl_code']}<|>{$posting_date}<|><|>{$doc_no}<|>{$soa_no}-{$customDescription}<|>({$amawnt})<|>{$company_code}<|>{$dept_code}<|>GENJNL<|>LEASING<|><|><|><|>{$formattedBillingPeriod};{$dueDateFormatted}<|><|>"];
                                            break;
                                        case 'PC000008':
                                    ...
                                    $amount = str_replace('-', '', $result['amount']);
                                    $rows[] = ["GENERAL<|>{$line_no}<|>G/L Account<|>{$result['gl_code']}<|>{$posting_date}<|><|>{$doc_no}<|>{$soa_no}-{$glAccountName}<|>({$amount})<|>{$company_code}<|>{$dept_code}<|>GENJNL<|>LEASING<|><|><|><|>{$formattedBillingPeriod};{$dueDateFormatted}<|><|>"];
                                }
            ...
            try {
                foreach ($ARreports['data'] as $key => $value) {
                    ...
                    if (!empty($data)) {
                        ...
                        $rows[] = ["GENERAL<|>$line_no<|>Customer<|>{$data[0]['tenant_id']}<|>$pDate<|><|>{$doc_no}-{$result['doc_no']}<|>{$data[0]['trade_name']}-Preop<|>{$preopAmount}<|>$company_code<|>$dept_code<|>GENJNL<|>LEASING<|><|><|><|>{$formattedBillingPeriod};{$dueDateFormatted}<|><|>"];
                        $line_no += 10000;
                        ...
                        foreach ($data as $result) {
                            ...
                            if ($result['description'] == 'Construction Bond') {
                                $rows[] = ["GENERAL<|>$line_no<|>G/L Account<|>10.20.01.01.03.10<|>$pDate<|><|>{$doc_no}-{$result['doc_no']}<|>{$result['description']}<|>({$result['amount']})<|>$company_code<|>$dept_code<|>GENJNL<|>LEASING<|><|><|><|>{$formattedBillingPeriod};{$dueDateFormatted}<|><|>"];
                            } else if ($result['description'] == 'Security Deposit - Kiosk and Cart' || $result['description'] == 'Security Deposit') {
                                $rows[] = ["GENERAL<|>$line_no<|>G/L Account<|>10.20.01.01.03.12<|>$pDate<|><|>{$doc_no}-{$result['doc_no']}<|>{$soa_no}-{$result['description']}<|>({$result['amount']})<|>$company_code<|>$dept_code<|>GENJNL<|>LEASING<|><|><|><|>{$formattedBillingPeriod};{$dueDateFormatted}<|><|>"];
                            } else if ($result['description'] == 'Advance Rent') {
                                $rows[] = ["GENERAL<|>$line_no<|>G/L Account<|>10.20.01.01.02.01<|>$pDate<|><|>{$doc_no}-{$result['doc_no']}<|>{$soa_no}-{$result['description']}<|>({$result['amount']})<|>$company_code<|>$dept_code<|>GENJNL<|>LEASING<|><|><|><|>{$formattedBillingPeriod};{$dueDateFormatted}<|><|>"];
                            }
    ...
    //                             //     if ($result['gl_code'] == '20.80.01.08.07') {
    //                             //         // Conditional description assignments
    //                             //         // dump($invoicing->charges_code);
    //                             //         switch ($invoicing->charges_code) {
    //                             //             case 'PC000007':
    //                             //                 $customDescription[] = 'Gas';
    //                             //                 break;
    ...
    //                             //         }
    //                             //     } else {
    //                             //         $customDescription[] = $glAccountName;
    //                             //     }

    //                             // }

    //                             // dump($customDescription);

    //                             // dump($result);
    //                             if (in_array($result['gl_code'], ['10.10.01.03.03', '10.10.01.03.04'])) {
    //                                 // $rows[] = ["GENERAL<|>{$line_no}<|>Customer<|>{$tenantID}<|>{$posting_date}<|><|>{$doc_no}<|>{$tradeName}-Other Charges<|>{$result['amount']}<|>{$company_code}<|>{$dept_code}<|>GENJNL<|>LEASING<|><|><|><|>{$soa->billing_period}<|><|>",];
    //                                 $rows[] = ["GENERAL<|>{$line_no}<|>Customer<|>{$tenantID}<|>{$posting_date}<|><|>{$doc_no}<|>{$tradeName}-Other Charges<|>{$result['amount']}<|>{$company_code}<|>{$dept_code}<|>GENJNL<|>LEASING<|><|><|><|>{$formattedBillingPeriod};{$dueDateFormatted}<|><|>",];
    //                             } elseif ($result['gl_code'] == '10.10.01.06.05') {
    //                                 // $rows[] = ["GENERAL<|>{$line_no}<|>G/L Account<|>{$result['gl_code']}<|>{$posting_date}<|><|>{$doc_no}<|>{$result['gl_account']}<|>{$result['amount']}<|>{$company_code}<|>{$dept_code}<|>GENJNL<|>LEASING<|><|><|><|>{$soa->billing_period}<|><|>",];
    //                                 $rows[] = ["GENERAL<|>{$line_no}<|>G/L Account<|>{$result['gl_code']}<|>{$posting_date}<|><|>{$doc_no}<|>{$glAccountName}<|>{$result['amount']}<|>{$company_code}<|>{$dept_code}<|>GENJNL<|>LEASING<|><|><|><|>{$formattedBillingPeriod};{$dueDateFormatted}|><|>",];
    ...
    //                                 $amount = str_replace('-', '', $result['amount']);
    //                                 // $rows[] = ["GENERAL<|>{$line_no}<|>G/L Account<|>{$result['gl_code']}<|>{$posting_date}<|><|>{$doc_no}<|>{$result['gl_account']}<|>({$amount})<|>{$company_code}<|>{$dept_code}<|>GENJNL<|>LEASING<|><|><|><|>{$soa->billing_period}<|><|>",];
    //                                 $rows[] = ["GENERAL<|>{$line_no}<|>G/L Account<|>{$result['gl_code']}<|>{$posting_date}<|><|>{$doc_no}<|>{$soa->soa_no}-{$glAccountName}<|>({$amount})<|>{$company_code}<|>{$dept_code}<|>GENJNL<|>LEASING<|><|><|><|>{$formattedBillingPeriod};{$dueDateFormatted}<|><|>",];
                        ...
                        if (!empty($data)) {
                            ...
                            ");
                                ...
                                if (in_array($result['gl_code'], ['10.10.01.03.03', '10.10.01.03.04'])) {
                                    ...
                                    $rows[] = ["GENERAL<|>{$line_no}<|>G/L Account<|>10.20.01.01.01.14<|>{$posting_date}<|><|>{$doc_no}<|>VAT Output<|>{$vat_amount}<|>{$company_code}<|>{$dept_code}<|>GENJNL<|>LEASING<|><|><|><|>{$formattedBillingPeriod};{$dueDateFormatted}|><|>",];
                                    ...
                                    switch ($invoicing->charges_code) {
                                        case 'PC000007':
                                            $customDescription = 'Gas';
                                            $rows[] = ["GENERAL<|>{$line_no}<|>G/L Account<|>{$result['gl_code']}<|>{$posting_date}<|><|>{$doc_no}<|>{$soa->soa_no}-{$customDescription}<|>{$amount_noVat}<|>{$company_code}<|>{$dept_code}<|>GENJNL<|>LEASING<|><|><|><|>{$formattedBillingPeriod};{$dueDateFormatted}|><|>"];
                                            break;
                                        case 'PC000008':
                                            $customDescription = 'Pest Control';
                                            $rows[] = ["GENERAL<|>{$line_no}<|>G/L Account<|>{$result['gl_code']}<|>{$posting_date}<|><|>{$doc_no}<|>{$soa->soa_no}-{$customDescription}<|>{$amount_noVat}<|>{$company_code}<|>{$dept_code}<|>GENJNL<|>LEASING<|><|><|><|>{$formattedBillingPeriod};{$dueDateFormatted}|><|>"];
                                    ...
                                    $rows[] = ["GENERAL<|>{$line_no}<|>G/L Account<|>{$result['gl_code']}<|>{$posting_date}<|><|>{$doc_no}<|>{$soa->soa_no}-{$glAccountName}<|>({$nv_amount})<|>{$company_code}<|>{$dept_code}<|>GENJNL<|>LEASING<|><|><|><|>{$formattedBillingPeriod};{$dueDateFormatted}<|><|>",];
            ...
            try {
                foreach ($ARreports['data'] as $key => $value) {
                    ...
                    if (!empty($data)) {
                        ...
                        $dueDateFormatted = date('M d, Y', strtotime($result['due_date']));
                        $formattedBillingPeriod = formatBillingPeriod($soa->billing_period);
                        $rows[] = ["GENERAL<|>$line_no<|>Customer<|>{$data[0]['tenant_id']}<|>$preopDate<|><|>{$doc_no}-{$result['doc_no']}<|>{$data[0]['trade_name']}-Preop<|>{$preopAmount}<|>$company_code<|>$dept_code<|>GENJNL<|>LEASING<|><|><|><|>{$formattedBillingPeriod};{$dueDateFormatted}<|><|>"];
                        $line_no += 10000;
                        ...
                        foreach ($data as $result) {
                            ...
                            if ($result['description'] == 'Construction Bond') {
                                ...
                                $rows[] = ["GENERAL<|>$line_no<|>G/L Account<|>10.20.01.01.03.12<|>$pDate<|><|>{$doc_no}-{$result['doc_no']}<|>$soa->soa_no-{$result['description']}<|>({$result['amount']})<|>$company_code<|>$dept_code<|>GENJNL<|>LEASING<|><|><|><|>{$formattedBillingPeriod};{$dueDateFormatted}<|><|>"];
                            } else if ($result['description'] == 'Advance Rent') {
                                ...
                                $rows[] = ["GENERAL<|>$line_no<|>G/L Account<|>10.20.01.01.02.01<|>$pDate<|><|>{$doc_no}-{$result['doc_no']}<|>{$soa->soa_no}-{$result['description']}<|>({$result['amount']})<|>$company_code<|>$dept_code<|>GENJNL<|>LEASING<|><|><|><|>{$formattedBillingPeriod};{$dueDateFormatted}<|><|>"];
                            }
    ...
    public function soaReprintPdf($file, $docno)
    {
        ...
        $collection_date = $this->uri->segment('5');
        $storeID = $this->session->userdata('user_group');
        $details = $this->app_model->get_soaReprintDetails($storeID, $soa_no);
        $soaDesc = $this->app_model->get_soaDescription($soa_no, $collection_date);
        ...
        $pdf->Rect(-50, -50, 270, 152, 'F'); // Draw a white rectangle covering the upper part of the page
        ...
        if (in_array($tenant_id, ['ICM-LT000008', 'ICM-LT000442', 'ICM-LT000492', 'ICM-LT000035', 'ICM-LT000120'])) {
            ...
            if ($store_name == 'ALTURAS MALL') {
                ...
                $pdf->cell(0, 5, 'Please make all checks payable to ' . strtoupper($details->store_name) . '', 0, 0, 'R');
            }
        }
        ...
        // $pdf->setFont('times', 'B', 10);
        // // $pdf->cell(100, 8, 'CURRENT(' . date('F Y', strtotime($details['collection_date'])) . ')', 0, 0, 'L');
        // $pdf->cell(100, 8, 'CURRENT(' . date('F Y', strtotime($soaDesc->due_date)) . ')', 0, 0, 'L');

        // if (!empty($details->current['basic'])) {
        //     $pdf->ln();
        //     $pdf->setFont('times', 'B', 10);
        //     $pdf->cell(10, 4, '     ', 0, 0, 'L');
        //     $pdf->cell(30, 4, 'Rental', 0, 0, 'L');
        ...
        // $pdf->cell(10, 4, '     ', 0, 0, 'L');
        // $pdf->cell(95, 4, 'Due Date : ', 0, 0, 'L');
        // $pdf->cell(25, 4, $soaDesc['soa_no'], 0, 0, 'R');
        // $pdf->cell(25, 4, '', 0, 0, 'R');
        // $pdf->cell(25, 4, '', 0, 0, 'R');
        // $pdf->ln();

        foreach ($soaDesc as $key => $doc) {
            $doc = (object) $doc;
            $pdf->setFont('times', 'B', 10);
            $pdf->cell(100, 8, 'CURRENT(' . date('F Y', strtotime($doc->posting_date)) . ')', 0, 0, 'L');
        ...
        $pdf->cell(20, 4, '     ', 0, 0, 'L');
        $pdf->cell(85, 4, $soaDesc->description, 0, 0, 'L');
        // $pdf->cell(25, 4, ($key == 0 ? 'P ' : '') . number_format($doc->amount, 2), 0, 0, 'R');
        // $pdf->cell(25, 4, '', 0, 0, 'R');
        // $pdf->cell(25, 4, '', 0, 0, 'R');
        // $pdf->ln();
        $this->db->trans_start();
