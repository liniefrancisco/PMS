...
class App_model extends CI_model
{
    ...
    function __construct()
    {
        ...
        $this->pis = $this->load->database('pis', true); // gwaps =================
    }
    ...
    public function check_login($username, $password) // added by gwaps ====================================
    {
        $exempted_names = ['Masterfile', 'IAD-ICM', 'Admin', 'LEASING'];
        $query = $this->db->query(
            "SELECT
                `u`.`id`,
                `u`.`password`,
        ...
        if ($query->num_rows() > 0) {
            ...
            foreach ($query->result() as $rows) {
                foreach ($exempted_names as $exempted_name) {
                    if (strpos($rows->name, $exempted_name) !== false) {
                        $data = array(
                            'id' => $rows->id,
                            'password' => $rows->password,
                            'user_type' => $rows->user_type,
                ...
                $employee_check = $this->pis->query(
                    "SELECT `name`, `current_status` 
                    FROM `employee3` 
                    WHERE LOWER(REPLACE(REPLACE(`name`, ',', ''), ' ', '')) = LOWER('" . str_replace([' ', ','], '', $rows->name) . "') 
                    AND `current_status` = 'Active'"
                );

                if ($employee_check->num_rows() == 0) {
                    return false;
                }
    ...
    public function add_store($company_code, $store_name, $dept_code, $store_code, $store_address, $contact_person, $contact_no, $email, $floors, $logo, $accre_bank, $managed_by, $tin)
    {
        $data = array(
    ...
    public function get_contractDocs($id) // revised by gwaps
    {
        $query = $this->db->query(
            //     "SELECT
            //         `file_name`
    ...
    public function get_contractDocs2($id, $term)
    {

        $query = $this->db->query("SELECT * FROM contract_docs 
                                   WHERE tenant_id = '$id' 
                                   AND file_name IN ('%new_contract%' OR '%renew_contract%' OR '%amended_contract%' OR '%award_notice%'
                                   AND term = '" . $term . "')
                                   ORDER BY `id` ASC");
    ...
    public function get_approveLprospect()
    {
        ...
        if ($this->session->userdata('user_type') == 'Administrator' || $this->session->userdata('user_type') == 'Corporate Documentation Officer' || $this->session->userdata('user_type') == 'Corporate Leasing Supervisor' || $this->session->userdata('user_type') == 'Legal') {
            $query = $this->db->query(
                "SELECT
                    `p`.`id`,
                    `p`.`trade_name`,
    ...
    public function get_sprospect()
    {
        $query = $this->db->query(
            ...
            AND
                `p`.`prepared_by` = `u`.`id`
            AND
                (`p`.`status` != 'On Contract' AND `p`.`status` != 'Denied' AND `p`.`status` != 'Revoked' AND `p`.`status` != 'Deleted')
            AND
                `p`.`flag` = 'Short Term'
            GROUP BY
                `p`.`id`
    ...
    public function generate_UTFTransactionNo($useNext = TRUE, $tinstatus)
    {
        $sequence = "";
    ...
    public function generate_ClosingRefNo($useNext = true, $tinstatus)
    {
        $sequence = [];
    ...
    public function generate_InternalTransactionNo($useNext = true, $tinstatus)
    {
        $sequence = "";
        ...
        if ($tinstatus === 'with') {
            $sequence = getSequenceNo(
                [
                    ...
                    'lpad' => '7',
                    'pad_string' => '0',
                    'description' => "Internal Payment Sequence No."
                ],
                [
                    'table' => 'subsidiary_ledger',
                    'column' => 'doc_no'
            ...
            $sequence = getSequenceNo(
                [
                    ...
                    'lpad' => '7',
                    'pad_string' => '0',
                    'description' => "Internal Payment Sequence No. no TIN"
                ],
                [
                    'table' => 'subsidiary_ledger',
                    'column' => 'doc_no'
    ...
    public function payment_docNo($useNext = true, $tinstatus)
    {
        /*$ret;
        ...
        if ($tinstatus === 'with') {
            $sequence = getSequenceNo(
                [
                    ...
                    'lpad' => '7',
                    'pad_string' => '0',
                    'description' => "Payment Doc No. Sequence"
                ],
                [
                    'table' => 'subsidiary_ledger',
                    'column' => 'doc_no'
            ...
            $sequence = getSequenceNo(
                [
                    ...
                    'lpad' => '7',
                    'pad_string' => '0',
                    'description' => "Using Preop Payment Doc No. Sequence"
                ],
                [
                    'table' => 'subsidiary_ledger',
                    'column' => 'doc_no'
    ...
    public function get_Ltenants()
    {
        ...
        if ($this->session->userdata('user_type') == 'Administrator' || $this->session->userdata('user_type') == 'Super Admin' || $this->session->userdata('user_type') == 'Corporate Leasing Supervisor' || $this->session->userdata('user_type') == 'Corporate Documentation Officer' || $this->session->userdata('user_type') == 'Legal') {
            $query = $this->db->query(
                "SELECT
                    `t`.`id`,
                    `t`.`tenant_id`,
    ...
    public function pending_lcontracts()
    {
        ...
        if ($this->session->userdata('user_type') == 'Administrator' || $this->session->userdata('user_type') == 'Super Admin' || $this->session->userdata('user_type') == 'Corporate Documentation Officer' || $this->session->userdata('user_type') == 'Corporate Leasing Supervisor' || $this->session->userdata('user_type') == 'Legal') {
            $query = $this->db->query(
                "SELECT
                    `t`.`id`,
                    `t`.`prospect_id`,
    ...
    { //LINIE UPDATED
        $query = $this->db->query(
            "SELECT
                    ...
                    `t`.`increment_frequency`,
                    `t`.`tin`,
                    (CASE WHEN t.is_vat = 'Added' THEN TRUE ELSE FALSE END) as is_vat,
                    (CASE WHEN t.wht = 'Added' THEN TRUE ELSE FALSE END) as wht,
                    `t`.`opening_date`,
                    `t`.`vat_agreement`,
                    `t`.`vat_percentage`,
                    `t`.`wht_percentage`,
                ...
                FROM
                    ...
                    LEFT JOIN `location_code` `lc` ON `t`.`locationCode_id` = `lc`.`id`
                    LEFT JOIN `location_slot` `ls` ON `lc`.`slots_id` = `ls`.`id`
                    LEFT JOIN `floors` `f` ON `lc`.`floor_id` = `f`.`id`
                    LEFT JOIN `stores` `s` ON `f`.`store_id` = `s`.`id`
                    LEFT JOIN `leasee_type` `lt` ON `p`.`lesseetype_id` = `lt`.`id`
                WHERE
                    `t`.`id` = '$id'
    ...
    public function get_otherCharges($tenant_id)
    {
        ...
        $query = $this->db->query(
            "SELECT
                *,
                IF(uom = 'Fixed Amount', 1, 0) total_unit,
                IF(uom = 'Fixed Amount', unit_price, 0) actual_amount,
                CAST(IFNULL((
                    SELECT 
                        i.curr_reading
                    FROM 
                        `invoicing` i
                    WHERE 
                        i.charges_code = `charges_setup`.`charges_code` 
                    AND 
                    ...
                    ORDER BY 
                        ...
                        i.id DESC
                    LIMIT 1
                ), 0)  AS UNSIGNED) as prev_reading,
                0 as curr_reading

            FROM
                charges_setup
            WHERE
                charges_type != 'Pre Operation Charges'
            AND 
                charges_type != 'Construction Materials'
            AND
               id NOT IN (SELECT `monthly_chargers_id` FROM `selected_monthly_charges` WHERE `tenant_id` = '$tenant_id')
    ...
    {
        $query = $this->db->query("SELECT * FROM `charges_setup` WHERE `charges_type` = 'Construction Materials'");
        return $query->result_array();
    }
    ...
    {
        $query = $this->db->query("SELECT * FROM `charges_setup` WHERE `charges_type` = 'Pre Operation Charges' ORDER BY `description` ASC");
        return $query->result_array();
    }
    ...
    public function getTenantByTenantID($tenant_id)
    {
        ...
        $query = $this->db->query(
            "SELECT
                ...
                `t`.`rent_percentage`,
                `t`.`basic_rental`,
                `t`.`basic_rental` AS org_basic_rental,
                `t`.`wht`,
                `t`.`is_vat`,
                `t`.`vat_agreement`,
                `t`.`tenancy_type`,
            ...
            LEFT JOIN
                `location_code` `l`
            ON
                (`t`.`locationCode_id` = `l`.`id` AND `l`.`status` = 'Active')
    ...
    public function select_tradeName($data, $tenancy_type)
    {
        ...
        $trade_name = $this->db->query(
            "SELECT
                ...
                `t`.`is_vat`,
                (CASE
                    WHEN (`t`.`is_vat` <> 'Added' AND `t`.`wht` <> 'Added') THEN ''
                    WHEN (`t`.`is_vat` = 'Added' OR `t`.`wht` = 'Added') AND `t`.`vat_agreement` = 'Inclusive' THEN 'Inclusive'
                    ELSE 'Exclusive'
                END) vat_agreement,
                `t`.`tenancy_type`,
                `t`.`tin`,
    ...
    public function get_docNo($useNext = true, $tinstatus)
    {
        $sequence = [];
    ...
    public function generate_refNo($useNext = true, $tinstatus)
    {
        $sequence = [];
    ...
    public function gl_refNo($useNext = true, $tinstatus)
    {
        $sequence = [];
    ...
    public function closingPDC_docNo($useNext = true)
    {
        ...
        $sequence = getSequenceNo(
            [
                ...
                'lpad' => '7',
                'pad_string' => '0',
                'description' => "PDC Closing Sequence No."
            ],
            [
                'table' => 'pdc_closing',
                'column' => 'doc_no'
    ...
    public function selected_monthly_charges($tenant_id)
    {
        ...
        $query = $this->db->query(
            "SELECT
                ...
                `charges`.`with_penalty`,
                `selected`.`uom`,
                `selected`.`unit_price`,
                IF(`selected`.`uom` = 'Fixed Amount', 1, 0) total_unit,
                IF(`selected`.`uom` = 'Fixed Amount', `selected`.`unit_price`, 0) actual_amount,
                CAST(IFNULL((
                    SELECT 
                        `curr_reading` 
                    FROM 
                        `invoicing` 
                    WHERE 
                        `charges_code` = `charges`.`charges_code` 
                    AND 
    ...
    //     public function get_myContract($id)
// {
//     $result = $this->db->query("SELECT `contract_no` FROM `tenants` WHERE `prospect_id` = '$id' AND `flag` = 'Pending' AND `status` = 'Active' LIMIT 1")->row();
//     if ($result) {
//         return $result->contract_no;
//     }
//     return null;
// }
    ...
    // public function get_myTenantID($id)
// {
//     $result = $this->db->query("SELECT `tenant_id` FROM `tenants` WHERE `prospect_id` = '$id' AND `flag` = 'Pending' AND `status` = 'Active' LIMIT 1")->row();
//     if ($result) {
...
// }
    ...
    public function generate_billing_date($tenant_id, $opening_date, $expiry_date)
    {
        ...
        if ($expiry_date <= strtotime(gmdate("Y-m-d", $opening_date) . " +" . $fetchLimit)) {
            ...
            while ($upperBound < $expiry_date) { // fetch full chunks while there're some left
    ...
    {
        $query = $this->db->query("SELECT *, 'Preop-Charges' as document_type FROM `tmp_preoperationcharges` WHERE `tenant_id` = '$tenant_id' AND (`tag` = '' || `tag` = 'Posted' AND amount >= 1)");
        return $query->result_array();
    }
    ...
    public function tenant_lookup($tenancy_type)
    {
        ...
        } elseif ($tenancy_type == 'Short Term Tenant') {
    ...
    public function get_unclearedPayment($tenant_id)
    {
        ...
        $query = $this->db->query("
            ...
            AND 
                (`sl`.`document_type` = 'Payment' OR `sl`.`document_type` = 'Payment Adjustment')
            GROUP BY
    ...
    public function get_clearedPayment($post_date, $tenant_id)
    {
        $query = $this->db->query("
            SELECT
                `sl`.`id`,
            ...
            FROM
                `subsidiary_ledger` `sl`
            LEFT JOIN (SELECT DISTINCT(tnt.prospect_id) AS prospect_id, tnt.tenant_id, tnt.store_code FROM tenants tnt) t 
            ON t.tenant_id = sl.tenant_id 
            ...
            ON
                `sl`.`gl_accountID` = `acc`.`id`
            WHERE
                DATE(`sl`.`posting_date`) LIKE '%$post_date%'
            AND
                `sl`.`tenant_id` = '$tenant_id'
            AND
                (`sl`.`status` = 'AR Clearing' OR `sl`.`status` = 'RR Clearing' OR `sl`.`status` = 'URI Clearing' OR `sl`.`status` = 'Preop Clearing')
    ...
    public function get_internalUnclearedPayment($tenant_id)
    {
        ...
        $query = $this->db->query("
            SELECT 
                ip.*,
                IFNULL(ip.amount, 0) as ip_amount,
                SUM(IFNULL(ri.amount, 0)) as ri_amount,
                IFNULL(ip.amount, 0) + SUM(IFNULL(ri.amount, 0)) as amount
            FROM
                (SELECT
                    `sl`.`id`,
                    `sl`.`ref_no`,
                    ...
                    `sl2`.`status` as status2,
                    `sl`.`ft_ref`,
                    SUM(IFNULL(`sl`.debit , 0) +  IFNULL(`sl`.credit , 0)) AS amount
                    FROM
                    `subsidiary_ledger` `sl`
                ...
                LEFT JOIN
                    ...
                    AND sl2.ref_no = sl.ref_no 
                    AND sl2.tenant_id = sl.tenant_id 
                    AND sl2.document_type IN ('Payment', 'Payment Adjustment')
                    AND ABS(sl2.credit) = ABS(sl.debit)
                    AND sl2.id <> sl.id)
                LEFT JOIN
                    `gl_accounts` `acc`
            ...
            LEFT JOIN
                ...
                GROUP BY
                    doc_no, ref_no, ft_ref
                ) AS ri
                ON  ip.doc_no <> ri.doc_no AND
                    ip.ref_no = ri.ref_no AND
                    ip.ft_ref = ri.ft_ref
                GROUP BY
                    ip.doc_no, ip.ref_no, ip.ft_ref
                HAVING 
                    amount >= 1
                ORDER BY
                    ip.posting_date ASC, ip.doc_no ASC
        ");
    ...
    public function get_storeDetails()
    {
        ...
        $query = $this->db->query("SELECT * FROM `stores` `s`, `leasing_users` `u` WHERE `u`.`user_group` = '$user_group' AND `u`.`user_group` = `s`.`id` LIMIT 1");
    ...
    public function get_paymentScheme($tenant_id)
    {
        $query = $this->db->query(
            ...
            WHERE `document_type` = 'Payment Adjustment'
    ...
    public function get_paymentScheme2($tenant_id)
    {
        $query = $this->db->query(
            "SELECT * FROM
                `payment_scheme`
        ...
        "
            //========================== gwaps ======================== 
            // "SELECT * FROM `payment_scheme` WHERE `tenant_id` = '$tenant_id' AND `status` IN ('NO OR', '') AND `receipt_no` NOT IN (
            //     SELECT `doc_no`
            //     FROM `subsidiary_ledger`
            //     WHERE `document_type` = 'Payment Adjustment'
            //     AND `tenant_id` = '$tenant_id')"
    ...
    public function wht_invoiceDebitMemo($tenant_id, $due_date, $doc_no, $amount)
    {
        $this->db->query(
            ...
            AND
                `tenant_id` = '$tenant_id'
            AND
                `description` = 'Creditable Withholding Taxes'
        "
        );
    }
    ...
    public function get_unclosedPDC($as_of)
    {
            ...
            $query = $this->db->query("
                ...
                FROM 
                    (SELECT
                        ...
                        sl.posting_date,
                        ps.check_no,
                        SUM(IFNULL(sl.debit, 0)) AS  debit,
                        SUM(IFNULL(sl.credit, 0)) AS credit,
                        SUM(IFNULL(sl.debit, 0) + IFNULL(sl.credit, 0)) AS amount
                    FROM
                        subsidiary_ledger sl
                    LEFT JOIN
                        gl_accounts a
                        ON sl.gl_accountID = a.id
                    LEFT JOIN
                        (SELECT distinct(prospect_id) as prospect_id, tenant_id, store_code  FROM tenants) t 
                    ...
                    LEFT JOIN
                        ...
                        ON p.id = t.prospect_id
                    LEFT JOIN
                        (SELECT receipt_no, check_date, tenant_id, check_no  FROM payment_scheme GROUP BY receipt_no, tenant_id) ps
                        ON sl.tenant_id  = ps.tenant_id AND sl.doc_no = ps.receipt_no
                    WHERE
                        a.gl_code = '10.10.01.03.07.01'
                    AND 
                        sl.tenant_id <> 'DELETED'
                    AND 
                        sl.document_type IN ('Payment', 'Payment Adjustment')
                    GROUP BY 
                        sl.ref_no, sl.tenant_id, sl.ft_ref
                    HAVING 
                        amount >= 1
                    ORDER BY 
                        sl.posting_date ASC) AS tbl
                WHERE 
                    check_date <= '$as_of'
                GROUP BY
                    tbl.doc_no, tbl.tenant_id
                HAVING 
                    amount >= 1    
                ORDER BY
                    tbl.check_date DESC
    ...
    {
        $query = $this->db->query("SELECT `doc_no` as `receipt_no` FROM `subsidiary_ledger` WHERE id = '$gl_id'")->row()->receipt_no;
        return $query;
    }
        ...
        $query = $this->db->query("
            ...
            FROM 
                ...
                LEFT JOIN
                    ...
                    ON sl.gl_accountID = a.id
                LEFT JOIN
                    (SELECT distinct(prospect_id) as prospect_id, tenant_id  FROM tenants) t 
                    ON sl.tenant_id =  t.tenant_id
                LEFT JOIN
                    prospect p
                    ON p.id = t.prospect_id
                ...
                AND 
                    sl.tenant_id <> 'DELETED'
                AND sl.document_type IN ('Payment', 'Payment Adjustment')
                GROUP BY sl.ref_no, sl.tenant_id, sl.ft_ref
                HAVING amount >= 1
                ORDER BY sl.posting_date ASC) AS tbl
            WHERE 
                tbl.doc_no='$receipt_no'
            GROUP BY
                tbl.doc_no, tbl.tenant_id 
            ORDER BY
                tbl.posting_date
    ...
    public function paymentList($user_id, $beginning_date, $end_date)
    {
        $query = $this->db->query(
            ...
            AND
                `p`.`tenant_id` NOT LIKE '%DELETED%' -- added by gwaps
            AND
                ...
                FROM
                    ...
                    FROM
                        (SELECT
                            ...
                            `gl`.`posting_date`,
                            `gl`.`due_date`,
                            ROUND(SUM(`gl`.`debit`) - IFNULL((SELECT SUM(abs(`gl1`.`credit`)) FROM `subsidiary_ledger` `gl1`, `gl_accounts` `acc1` WHERE `gl`.`ref_no` = `gl1`.`ref_no` AND  (`acc1`.`gl_code` = '10.10.01.01.02' OR `acc1`.`gl_code` = '10.10.01.03.16' OR `acc1`.`gl_code` = '10.10.01.01.01') AND `gl1`.`posting_date` <= '$date' AND `gl1`.`gl_accountID` = `acc1`.`id` AND (`gl1`.`status` != 'Cleared' OR `gl1`.`status` IS NULL)) ,0), 2) AS `balance`
                        FROM
                        ...
                        AND
                            ROUND(`gl`.`debit` - IFNULL((SELECT SUM(abs(`gl1`.`credit`)) FROM `subsidiary_ledger` `gl1`, `gl_accounts` `acc1` WHERE `gl`.`ref_no` = `gl1`.`ref_no`  AND (`acc1`.`gl_code` = '10.10.01.01.02' OR `acc1`.`gl_code` = '10.10.01.03.16' OR `acc1`.`gl_code` = '10.10.01.01.01') AND `gl1`.`posting_date` <= '$date' AND `gl1`.`gl_accountID` = `acc1`.`id` AND (`gl1`.`status` != 'Cleared' OR `gl1`.`status` IS NULL)) ,0), 2) > 1
                ...
                FROM
                    ...
                    FROM
                        (SELECT
                            ...
                            `gl`.`posting_date`,
                            `gl`.`due_date`,
                            ROUND(SUM(`gl`.`debit`) - IFNULL((SELECT SUM(abs(`gl1`.`credit`)) FROM `subsidiary_ledger` `gl1`, `gl_accounts` `acc1` WHERE `gl`.`ref_no` = `gl1`.`ref_no` AND  (`acc1`.`gl_code` = '10.10.01.01.02' OR `acc1`.`gl_code` = '10.10.01.03.03' OR `acc1`.`gl_code` = '10.10.01.03.04') AND `gl1`.`posting_date` <= '$date' AND `gl1`.`gl_accountID` = `acc1`.`id` AND (`gl1`.`status` != 'Cleared' OR `gl1`.`status` IS NULL)) ,0), 2) AS `balance`
                        FROM
                            `subsidiary_ledger` `gl`,
                            (SELECT t.tenant_id, p.trade_name FROM tenants t, prospect p WHERE t.store_code = '" . $this->session->userdata('store_code') . "' AND t.tenant_id != 'DELETED' AND t.prospect_id =  p.id GROUP BY t.tenant_id) AS t
                        WHERE
                        ...
                        AND
                            (`gl`.`tag` = 'Other' OR `gl`.`tag` = 'Penalty')
                        AND
                            ROUND(`gl`.`debit` - IFNULL((SELECT SUM(abs(`gl1`.`credit`)) FROM `subsidiary_ledger` `gl1`, `gl_accounts` `acc1` WHERE `gl`.`ref_no` = `gl1`.`ref_no`  AND (`acc1`.`gl_code` = '10.10.01.01.02' OR `acc1`.`gl_code` = '10.10.01.03.03' OR `acc1`.`gl_code` = '10.10.01.03.04') AND `gl1`.`posting_date` <= '$date' AND `gl1`.`gl_accountID` = `acc1`.`id` AND (`gl1`.`status` != 'Cleared' OR `gl1`.`status` IS NULL)) ,0), 2) > 1
                        GROUP BY `gl`.`ref_no`, `gl`.`due_date`) AS report) as rr
                    GROUP BY rr.tenant_id
                    ORDER BY rr.trade_name ASC
            "
    ...
    //         AND
    //             (gl.document_type = 'Invoice' OR gl.document_type = 'Invoice Adjustment')
    //         AND
    ...
    { //LINIE
        ...
        //             gl.credit,
        //             SUM(gl.debit) 
        //                 - IFNULL((SELECT SUM(ABS(gl1.credit)) FROM subsidiary_ledger gl1 WHERE gl.ref_no = gl1.ref_no AND gl1.adj_tag = 'Credit Memo' AND gl1.gl_accountID = gl.gl_accountID), 0) 
        //                 - IFNULL((SELECT SUM(ABS(gl2.credit)) FROM subsidiary_ledger gl2 WHERE gl.ref_no = gl2.ref_no AND gl2.document_type = 'Payment' AND gl2.gl_accountID = gl.gl_accountID), 0) AS amount
        //         FROM
        //             subsidiary_ledger gl
        //         JOIN
        //             (SELECT t.tenant_id, p.trade_name FROM tenants t JOIN prospect p ON t.prospect_id = p.id WHERE t.store_code = '" . $this->session->userdata('store_code') . "' AND t.tenant_id != 'DELETED' GROUP BY t.tenant_id) AS tenants
        //         ON
        //             tenants.tenant_id = gl.tenant_id
        //         WHERE
        //             (gl.gl_accountID = '4' OR gl.gl_accountID = '22' OR gl.gl_accountID = '29')
        //         AND
        //             (gl.document_type = 'Invoice' OR gl.document_type = 'Invoice Adjustment' OR gl.document_type = 'Payment')
        //         AND
        //             DATE_FORMAT(gl.posting_date, '%M %Y') = '$month'
        //         GROUP BY
        //             gl.ref_no, gl.tenant_id
        //         ) AS report
        //     GROUP BY
        //         report.tenant_id
        // ")
        ...
        $query = $this->db->query("
            SELECT
                ...
                report.trade_name,
                report.gl_accountID,
                MAX(CASE WHEN report.gl_accountID = '4' THEN report.debit ELSE 0 END) AS basic_amount,
                MAX(CASE WHEN report.gl_accountID = '22' THEN report.debit ELSE 0 END) AS other_amount,
                SUM(report.amount) AS amount
            FROM
                (SELECT
                    gl.tenant_id,
    ...
    public function generate_monthly_receivable_summary_basic($month)
    {
        $query = $this->db->query(
            "SELECT
                    report.tenant_id,
    ...
    public function generate_monthly_receivable_summary_other($month)
    {
        $query = $this->db->query(
            "SELECT
                    report.tenant_id,
                ...
                FROM
                    ...
                    FROM
                        ...
                        (SELECT t.tenant_id, p.trade_name FROM tenants t, prospect p WHERE t.store_code = '" . $this->session->userdata('store_code') . "' AND t.tenant_id != 'DELETED' AND t.prospect_id =  p.id GROUP BY t.tenant_id) AS tenants
                    WHERE
                        (gl.gl_accountID = '22' OR gl.gl_accountID = '29')
                    AND
                        (gl.document_type = 'Invoice' OR gl.document_type = 'Invoice Adjustment')
                    AND
                        DATE_FORMAT(gl.posting_date, '%M %Y') = '$month'
    ...
    public function generate_monthly_receivable_summary_new($month)
    {
        $query = $this->db->query("
            SELECT
                ...
                report.gl_accountID,
                report.tin,
                SUM(report.basic) AS basic_amount,
                SUM(report.others) AS others_amount,
                SUM(report.basic + report.others) AS total_amount
            FROM
                (SELECT
                    gl.tenant_id,
                    tenants.trade_name,
                    ...
                    gl.gl_accountID,
                    CASE 
                        WHEN gl.gl_accountID = '4' THEN SUM(gl.debit) - IFNULL((
                            SELECT SUM(ABS(gl1.credit)) 
                            FROM subsidiary_ledger gl1 
                            WHERE gl.ref_no = gl1.ref_no 
                            AND gl1.gl_accountID = gl.gl_accountID
                        ), 0)
                        ELSE 0
                    END AS basic,
                    CASE 
                        WHEN gl.gl_accountID IN ('22', '29') THEN SUM(gl.debit) - IFNULL((
                            SELECT SUM(ABS(gl1.credit)) 
                            FROM subsidiary_ledger gl1 
                            WHERE gl.ref_no = gl1.ref_no 
                            AND gl1.gl_accountID = gl.gl_accountID
                ...
                INNER JOIN
                    ...
                    FROM 
                        tenants t 
                    INNER JOIN 
                        prospect p ON t.prospect_id = p.id
                    WHERE 
                        t.store_code = '" . $this->session->userdata('store_code') . "' 
                        AND t.tenant_id != 'DELETED'
                    GROUP BY 
                        t.tenant_id) AS tenants ON tenants.tenant_id = gl.tenant_id
                WHERE
                    gl.gl_accountID IN ('4', '22', '29')
                AND
                    (gl.document_type = 'Invoice' OR gl.document_type = 'Invoice Adjustment')
                AND
                    DATE_FORMAT(gl.posting_date, '%M %Y') = '$month'
                GROUP BY
                    gl.ref_no, gl.gl_accountID) AS report
            GROUP BY 
                report.tenant_id, report.gl_accountID, report.tin
            HAVING 
                SUM(report.basic + report.others) > 0 
            ORDER BY 
                report.trade_name ASC
        ");
        return $query->result_array();
    }
    // --------------------------------------------
    ...
    // public function AR_monthly_total_new($month)
    // {
    ...
    // public function RR_monthly_total_new($month)
    // {
    ...
    public function generate_ExportNo_internal($useNext = TRUE)
    {
        $sequence = getSequenceNo(
            [
                'code' => "EXPI",
                ...
                'lpad' => '7',
                'pad_string' => '0',
                'description' => "Nav Exportation Sequence Internal"
            ],
            [
                'table' => 'exportation_log',
                'column' => 'batch_code'
    ...
    // {
    //     $this->db->update('subsidiary_ledger', ['export_batch_code' => $batch_code], "doc_no = '$doc_no' AND (export_batch_code IS NULL OR export_batch_code = '')");
    // }

    public function updateEntryAsExported($table, $data, $where)
    {
        $this->db->update($table, $data, $where);
    }

    public function updateEntryAsExportedPreop($doc_no, $batch_code)
    {
        $this->db->update('tmp_preoperationcharges', ['export_batch_code' => $batch_code], "doc_no = '$doc_no' AND (export_batch_code IS NULL OR export_batch_code = '')");
    }
    ...
    {
        $store = $this->session->userdata('store_code');
        $data = $this->db->select(['l.id', 'l.batch_code', 'l.type', 'l.filter', 'l.store', 'l.exported_at', "u.name as full_name"])
    ...
    // public function generate_RRreports($month, $tosearch)
    // {
    //     $doc_nos = [];
    //     $store = $this->session->userdata('store_code');
    //     $invoices = $this->db->query("SELECT doc_no, ref_no
    //                                   FROM subsidiary_ledger
    //                                   WHERE DATE_FORMAT(posting_date, '%M %Y') = '$month'
    //                                   AND gl_accountID = '4' 
    //                                   AND (tenant_id <> 'DELETED' AND ref_no <> 'DELETED' AND doc_no <> 'DELETED')
    //                                   AND tenant_id <> 'ICM-LT000064'
    //                                   {$tosearch}
    //                                   AND (document_type = 'Invoice' OR document_type = 'Invoice Adjustment')
    //                                   AND (export_batch_code IS NULL OR export_batch_code = '')
    //                                   GROUP BY doc_no")->result();
    ...
    //                                     SUM(IFNULL(g.debit, 0)) debit, 
    //                                     SUM(IFNULL(g.credit, 0)) credit,
    //                                     SUM(IFNULL(g.debit, 0) + IFNULL(g.credit, 0)) as amount 
    //                                     FROM subsidiary_ledger as g 
    //                                     LEFT JOIN (SELECT distinct(tnt.prospect_id) as prospect_id, tnt.tenant_id, tnt.store_code FROM tenants tnt) t ON t.tenant_id = g.tenant_id 
    //                                     LEFT JOIN prospect p on p.id = t.prospect_id LEFT JOIN gl_accounts a ON a.id = g.gl_accountID 
    //                                     WHERE g.doc_no IN ('$docs') 
    //                                     AND (g.gl_accountID = '6' OR g.gl_accountID = '4' OR g.gl_accountID = '11' OR (g.gl_accountID = '5' AND (g.tag IS NULL OR g.tag = ''))) 
    //                                     AND (g.document_type = 'Invoice' OR g.document_type = 'Invoice Adjustment') 
    //                                     AND (g.tenant_id <> 'DELETED' AND g.ref_no <> 'DELETED' AND g.doc_no <> 'DELETED') 
    //                                     AND g.tenant_id != 'ICM-LT000064' 
    //                                     AND t.store_code = '$store' 
    //                                     GROUP BY g.id
    //                                     ORDER BY g.tenant_id, g.doc_no, g.gl_accountID")->result_array();
    ...
    public function generate_RRreports($month, $tosearch)
    {
        ...
        if (!$invoices) {
            log_message('error', 'Failed to fetch invoices: ' . $this->db->error()['message']);
            return false;
        }
        ...
        $docs = implode("','", $doc_nos);
        $data_query = $this->db->query("SELECT g.id, 
                                        TRIM(g.tenant_id) tenant_id, 
                                        TRIM(g.posting_date) posting_date, 
                                        TRIM(g.due_date) due_date, 
                                        TRIM(g.doc_no) doc_no,
                                    ...
                                    LEFT JOIN soa_line s ON s.doc_no = g.doc_no  
                                    WHERE g.doc_no IN ('$docs') 
                                    AND (g.gl_accountID = '6' OR g.gl_accountID = '4' OR g.gl_accountID = '11' OR (g.gl_accountID = '5' AND (g.tag IS NULL OR g.tag = ''))) 
                                    AND (g.document_type = 'Invoice' OR g.document_type = 'Invoice Adjustment') 
                                    AND (g.tenant_id <> 'DELETED' AND g.ref_no <> 'DELETED' AND g.doc_no <> 'DELETED') 
                                    ...
                                    AND t.store_code = '$store' 
                                    GROUP BY g.id
                                    ORDER BY g.tenant_id, g.doc_no, g.gl_accountID");
        if (!$data_query) {
            log_message('error', 'Failed to fetch data: ' . $this->db->error()['message']);
    ...
    public function checkBalance($doc_no, $date)
    {
        return $this->db->query("SELECT  
                                    ...
                                    inv.ref_no,
                                    (CASE
                                        WHEN inv.gl_accountID = '4' AND inv.tag = 'Retro Rent'  THEN CONCAT('Basic-', p.trade_name, '-Retro Rent')
                                        WHEN inv.gl_accountID = '4' THEN CONCAT('Basic-', p.trade_name)
                                        WHEN inv.gl_accountID IN (22,29) AND inv.tag = 'Penalty' THEN CONCAT('Other-', p.trade_name,'-Penalty')
                                        ELSE CONCAT('Other-', p.trade_name) 
                                    END) AS description,
                                    inv.tag,
                                    inv.gl_accountID,
                                    inv.tenant_id,                
                                    SUM(IFNULL(inv.amount,0)) as invoice_amount,
                                    SUM(IFNULL(memo.amount, 0)) as adj_amount,
                                    SUM(IFNULL(inv.amount, 0) + IFNULL(memo.amount, 0)) as debit,
                                    ABS(SUM(IFNULL(np.amount, 0))) AS nopenalty_amount
                                FROM
                                ...
                                LEFT JOIN
                                    (SELECT tenant_id,
                                            ...
                                            SUM(IFNULL(debit,0) + IFNULL(credit,0)) as amount
                                    FROM subsidiary_ledger
                                    WHERE gl_accountID NOT IN (22,29,4)
                                    AND document_type = 'Invoice'
                                    AND with_penalty = 'No'
                                    AND tenant_id <> 'DELETED'
                                    GROUP BY ref_no, doc_no, tenant_id
                                    ) AS np
                                    ON  inv.ref_no = np.ref_no AND 
                                        inv.doc_no = np.doc_no  AND 
                                        inv.tenant_id = np.tenant_id AND
                                        inv.gl_accountID <> 4
                                LEFT JOIN
                                    (SELECT 
                                        distinct(tnt.prospect_id) as prospect_id, 
                                        tnt.tenant_id, tnt.store_code  
                                    FROM 
                                        tenants tnt) 
                                    AS t 
                                    ON t.tenant_id = inv.tenant_id
                                LEFT JOIN 
                                    prospect p 
                                    ON p.id = t.prospect_id
                                GROUP BY 
    ...
    public function generate_ARreports($month, $tosearch)
    {
        ...
        $docs = $doc_nos;
        $docs = implode("','", $doc_nos);
        $data = $this->db->query("SELECT sl.id, 
                                         TRIM(sl.tenant_id) tenant_id,
                                         TRIM(p.trade_name) trade_name,
                                         sl.posting_date,
                                         sl.due_date,
                                         TRIM(sl.doc_no) doc_no,
                                         TRIM(sl.cas_doc_no) cas_doc_no,
                                         ...
                                         (CASE 
                                            ...
                                            ELSE SUM(sl.credit) + (SELECT IFNULL(SUM(sl1.debit), 0) FROM subsidiary_ledger sl1 WHERE sl.ref_no = sl1.ref_no AND sl1.gl_accountID = sl.gl_accountID AND sl1.document_type = 'Credit Memo' AND sl1.tenant_id != 'DELETED')
                                         END) AS amount
                                         FROM subsidiary_ledger AS sl
                                         LEFT JOIN (SELECT DISTINCT (tnt.prospect_id) AS prospect_id,
                                                           tnt.tenant_id,
                                                           tnt.store_code
                                                           FROM tenants tnt) t ON t.tenant_id = sl.tenant_id
                                         LEFT JOIN prospect p ON p.id = t.prospect_id
                                         LEFT JOIN gl_accounts ga ON ga.id = sl.gl_accountID
                                         WHERE sl.gl_accountID = ga.id
                                         AND (sl.gl_accountID = '12' OR sl.gl_accountID = '13' OR sl.gl_accountID = '14' OR sl.gl_accountID = '15' OR sl.gl_accountID = '16' OR sl.gl_accountID = '17' OR sl.gl_accountID = '18' OR sl.gl_accountID = '20' OR sl.gl_accountID = '22' OR sl.gl_accountID = '29' OR sl.gl_accountID = '30' OR ( sl.gl_accountID = '5' AND sl.tag = 'Expanded'))
                                         AND (sl.document_type = 'Invoice' OR sl.document_type = 'Invoice Adjustment')
                                         AND DATE_FORMAT(sl.posting_date, '%M %Y') = '$month'
                                         AND (sl.tenant_id <> 'DELETED' AND sl.ref_no <> 'DELETED' AND sl.doc_no <> 'DELETED')
                                         AND sl.doc_no IN ('$docs')
                                         AND (sl.export_batch_code IS NULL OR sl.export_batch_code = '')
                                         AND sl.tenant_id IN (SELECT t.tenant_id FROM tenants t, prospect p where  t.store_code = '$store' AND t.tenant_id != 'ICM-LT000064' AND t.prospect_id = p.id group by tenant_id)
                                         GROUP BY sl.id ORDER BY sl.tenant_id, amount DESC")->result_array();
    ...
    public function generate_RRreportsAdjustment($month, $tenantid)
    {
        ...
        $invoices = $this->db->query("SELECT doc_no, ref_no
                                      ...
                                      AND (document_type = 'Invoice Adjustment')
                                      AND (export_batch_code IS NULL OR export_batch_code = '')
                                      GROUP BY doc_no")->result();
        ...
        $data = $this->db->query("SELECT g.id, 
                                        ...
                                        WHERE g.doc_no IN ('$docs') 
                                        AND (g.gl_accountID = '6' OR g.gl_accountID = '4' OR g.gl_accountID = '11' OR (g.gl_accountID = '5' AND (g.tag IS NULL OR g.tag = ''))) 
                                        AND (g.document_type = 'Invoice Adjustment') 
                                        AND (g.tenant_id <> 'DELETED' AND g.ref_no <> 'DELETED' AND g.doc_no <> 'DELETED') 
                                        AND g.tenant_id != 'ICM-LT000064' 
                                        AND t.store_code = '$store' 
                                        GROUP BY g.id
    ...
    public function generate_ARreportsAdjustment($month, $tenantid)
    {
        #sl.tenant_id IN (SELECT t.tenant_id FROM tenants t, prospect p where  t.store_code = '$store' AND t.tenant_id != 'ICM-LT000064' AND t.prospect_id = p.id group by tenant_id)
        $store = $this->session->userdata('store_code');
        $invoices = $this->db->query("SELECT 
                                                sl.doc_no
                                            FROM
                                                subsidiary_ledger sl
                                            WHERE
                                                (sl.document_type = 'Invoice Adjustment')
                                                    AND (sl.gl_accountID = '12'
                                                    OR sl.gl_accountID = '13'
                                                    OR sl.gl_accountID = '14'
                                                    OR sl.gl_accountID = '15'
                                                    OR sl.gl_accountID = '16'
                                                    OR sl.gl_accountID = '17'
                                                    OR sl.gl_accountID = '18'
                                                    OR sl.gl_accountID = '20'
                                                    OR sl.gl_accountID = '22'
                                                    OR sl.gl_accountID = '29'
                                                    OR sl.gl_accountID = '30'
                                                    OR (sl.gl_accountID = '5'
                                                    AND sl.tag = 'Expanded'))
                                                    AND DATE_FORMAT(sl.posting_date, '%M %Y') = '$month'
                                                    AND (sl.export_batch_code IS NULL
                                                    OR sl.export_batch_code = '')
                                                    AND (sl.tenant_id <> 'DELETED'
                                                    AND sl.ref_no <> 'DELETED'
                                                    AND sl.doc_no <> 'DELETED')
                                                    AND sl.tenant_id LIKE '%$tenantid%'
                                            GROUP BY sl.doc_no , sl.tenant_id")->result();
        ...
        $data = $this->db->query("SELECT 
                                        ...
                                        (CASE
                                            ...
                                            THEN
                                                SUM(sl.debit) - (SELECT 
                                                        IFNULL(SUM(ABS(sl1.credit)), 0)
                                                    FROM
                                                        subsidiary_ledger sl1
                                                    WHERE
                                                        sl.ref_no = sl1.ref_no
                                                            AND sl1.gl_accountID = sl.gl_accountID
                                                            AND sl1.document_type = 'Credit Memo'
                                                            AND sl1.tenant_id != 'DELETED')
                                            ELSE SUM(sl.credit) + (SELECT 
                                                    IFNULL(SUM(sl1.debit), 0)
                                                FROM
                                                    subsidiary_ledger sl1
                                                WHERE
                                                    sl.ref_no = sl1.ref_no
                                        ...
                                        END) AS amount
                                    FROM
                                        subsidiary_ledger AS sl
                                            LEFT JOIN
                                        (SELECT DISTINCT
                                            (tnt.prospect_id) AS prospect_id,
                                                tnt.tenant_id,
                                                tnt.store_code
                                        FROM
                                            tenants tnt) t ON t.tenant_id = sl.tenant_id
                                            LEFT JOIN
                                        prospect p ON p.id = t.prospect_id
                                            LEFT JOIN
                                        gl_accounts ga ON ga.id = sl.gl_accountID
                                    WHERE
                                        sl.gl_accountID = ga.id
                                            ...
                                            OR (sl.gl_accountID = '5'
                                            AND sl.tag = 'Expanded'))
                                            AND (sl.document_type = 'Invoice Adjustment')
                                            AND DATE_FORMAT(sl.posting_date, '%M %Y') = '$month'
                                            AND (sl.tenant_id <> 'DELETED'
                                            AND sl.ref_no <> 'DELETED'
                                            AND sl.doc_no <> 'DELETED')
                                            ...
                                            WHERE
                                                t.store_code = '$store'
                                                    AND t.tenant_id != 'ICM-LT000064'
                                                    AND t.prospect_id = p.id
                                            GROUP BY tenant_id)
                                    GROUP BY sl.id
                                    ORDER BY sl.tenant_id , amount DESC")->result_array();
    ...
    //     } else {
    //         log_message('error', 'Query failed in generate_paymentCollection: ' . $this->db->last_query());
    //         return [];
    //     }
    // }
    ...
    public function generate_PreOpReports($month, $toSearch)
    {
        $store = $this->session->userdata('store_code');
        $invoices = $this->db->query("SELECT doc_no
                                      FROM tmp_preoperationcharges
                                      WHERE DATE_FORMAT(posting_date, '%M %Y') = '$month'
                                      AND (tenant_id <> 'DELETED' AND doc_no <> 'DELETED')
                                      AND tenant_id <> 'ICM-LT000064'
                                      {$toSearch}
                                      AND (export_batch_code IS NULL OR export_batch_code = '')
                                      GROUP BY doc_no")->result();
        ...
        $data = $this->db->query("SELECT g.id, 
                                        ...
                                        TRIM(g.doc_no) doc_no,
                                        TRIM(g.cas_doc_no) cas_doc_no,
                                        TRIM(g.description) description,
                                        TRIM(p.trade_name) trade_name,
                                        SUM(IFNULL(g.org_amount, 0)) as amount 
                                  FROM tmp_preoperationcharges as g 
                                  LEFT JOIN (SELECT distinct(tnt.prospect_id) as prospect_id, tnt.tenant_id, tnt.store_code FROM tenants tnt) t ON t.tenant_id = g.tenant_id 
                                  LEFT JOIN prospect p on p.id = t.prospect_id
                                  WHERE g.doc_no IN ('$docs') 
                                  AND (g.tenant_id <> 'DELETED' AND g.doc_no <> 'DELETED') 
                                  AND g.tenant_id != 'ICM-LT000064' 
                                  AND t.store_code = '$store' 
                                  GROUP BY g.id
                                  ORDER BY g.tenant_id, g.doc_no")->result_array();
        return compact('data', 'doc_nos');
    }
            ...
            "
                    ...
                    FROM 
                        ...
                        WHERE
                            ...
                            OR sl.gl_accountID = '24'
                            OR sl.gl_accountID = '25'
                            OR sl.gl_accountID = '31'
                            OR sl.gl_accountID = '32')
                        AND
                            (sl.document_type = 'Payment' OR sl.document_type = 'Payment Adjustment')
                        AND 
                    ...
                    LEFT JOIN
                        ...
                        FROM 
                            subsidiary_ledger s 
                        WHERE 
                            (s.debit IS NULL OR s.debit = 0 )
                        AND 
                            (s.credit IS NOT NULL OR s.credit <> 0) 
                        AND 
                            (s.document_type = 'Payment' OR s.document_type = 'Payment Adjustment')
                        GROUP BY 
    ...
    public function generate_paymentCollection_($month)
    {
        if ($this->_user_group == '' || $this->_user_group == '0' || $this->_user_group == NULL) {
            $query = $this->db->query(
                "SELECT
    ...
    public function getTenantBalances($tenant_id, $date_created)
    {
        $query = $this->db->query("
            SELECT  
                (CASE 
    ...
    public function getTenantPreopBalances($tenant_id, $date_created)
    {
        $query = $this->db->query("
            SELECT 
                id,
                'Preop-Charges' as document_type,
                doc_no,
                cas_doc_no,
                description,
                posting_date,
                due_date,
                IFNULL(org_amount, amount)  as debit,
                (CASE 
                    WHEN org_amount IS NULL THEN 0
                    ELSE (org_amount - amount) * -1 
                END) AS credit,
                amount as balance
            FROM 
    ...
    public function getInvoicingData($tenant_id, $doc_no)
    {
    ...
    public function getTenantUnearnedRentIncome($tenant_id, $posting_date)
    {
        $query = $this->db->query("
            SELECT 
                doc_no,
                ...
                posting_date,
                gl_accountID,
                SUM(ABS(IFNULL(debit,0))) as debit,
                SUM(ABS(IFNULL(credit,0))) as credit,
                SUM(ABS(IFNULL(credit,0))) - SUM(ABS(IFNULL(debit,0))) as balance
            FROM
                subsidiary_ledger
            WHERE
                gl_accountID = 7
            AND
                tenant_id = '$tenant_id'
            GROUP BY 
                ref_no, tenant_id, gl_accountID
            having 
                (balance > .10 OR balance >= 1) AND posting_date <= '$posting_date'
        ")->result_object();

        return $query;
    }

    public function get_soaPreviousBilling($tenant_id, $posting_date)
    {
        $query = $this->db->query(
            "SELECT
                s.tenant_id,
                IFNULL(s.amount_payable, 0) as amount_payable,
            ...
            LEFT JOIN
                ...
                ON 
                    s.tenant_id = p.tenant_id AND s.soa_no = p.soa_no
            WHERE
                s.tenant_id = '$tenant_id'
            AND
                s.posting_date < '$posting_date'
            ORDER BY
                s.posting_date DESC, s.id DESC
            LIMIT 1
        "
        );
    ...
    public function getSoaWithBalances($tenant_id, $posting_date)
    {
        $query = $this->db->query(
            ...
            WHERE 
                s.tenant_id = '$tenant_id' AND f.posting_date <= '$posting_date'
            GROUP BY
                s.soa_no, s.tenant_id
            HAVING 
                balance >= 1
            ORDER BY f.posting_date DESC, s.id DESC"
        )->result_object();


        /*$soa_docs = array_map(function($soa){
            return $soa->soa_no;
        }, $query);*/
    ...
    public function getInvoicesBySoaNo($tenant_id, $soa_no, $pmt_type = '')
    {
        ...
        foreach ($docs as $key => $doc) {
            if ($doc->preop_id) {
                //IF FOR PAYMENT USING PREOP, DONT INCLUDE PREOP INVOICES
                if ($pmt_type != 'preop_payment') {
                    $preops[] = $doc->preop_id;
                }
            } else {
                $invs[] = $doc->doc_no;
        ...
        $inv_docnos = "'" . implode("','", $invs) . "'";
        $preops_ids = "'" . implode("','", $preops) . "'";
        ...
        $invoices = $this->db->query("
            ...
            HAVING
                (balance > .10 OR balance >= 1)
            ORDER BY 
                inv.posting_date ASC, inv.gl_accountID ASC, inv.id ASC
        ")->result_object();

        /* ====================================================================
        SUM(IFNULL(adj.amount,0)) as pmt_adj,
        SUM(IFNULL(p.debit,0) + IFNULL(p.credit,0)) + SUM(IFNULL(adj.amount,0)) as amount
        LEFT JOIN 
            (SELECT tenant_id,
                doc_no,
                ref_no,
            ...
            WHERE document_type = 'Payment Adjustment'
            AND tenant_id <> 'DELETED'
            GROUP BY ref_no, gl_accountID, tenant_id, ft_ref) AS  adj
            ON p.ref_no = adj.ref_no AND p.doc_no = adj.adj_ref AND p.gl_accountID = adj.gl_accountID AND p.tenant_id = adj.tenant_id
        ==============================================================================================================================*/

        $preop_charges = $this->db->query("
            SELECT 
                id,
                'Preop-Charges' as document_type,
                doc_no,
        ...
        $data = array_merge($preop_charges, $invoices);

        usort($data, function ($d1, $d2) {
            $date1 = strtotime($d1->posting_date);
            $date2 = strtotime($d2->posting_date);

            if ($date1 == $date2) {
                if (!empty($d2->gl_accountID) && !empty($d1->gl_accountID) && $d2->gl_accountID == 4 && $d1->gl_accountID == 4) {
                    return 0;
                } elseif (!empty($d2->gl_accountID) && !empty($d1->gl_accountID) && $d2->gl_accountID == 4 && $d1->gl_accountID != 4) {
                    return 1;
                } elseif (!empty($d2->gl_accountID) && !empty($d1->gl_accountID)) {
                    return 0;
                } elseif (empty($d2->gl_accountID) && !empty($d1->gl_accountID)) {
                    return 1;
                } else {
    ...
    function getAccreditedBanks()
    {

        $store_code = $this->session->userdata['store_code'];
        $query = $this->db->query("SELECT * FROM accredited_banks WHERE store_code = '$store_code'");
        return $query->result_object();
    }

    public function get_soaDetails($tenant_id, $soa_no)
    {
    ...
    public function checkPaymentReceiptExistence($receipt_no)
    {
    ...
    public function getLedgerFirstResultByDocNo($tenant_id, $doc_no)
    {
        $query = $this->db->query("
            SELECT 
                * 
    ...
    public function getLastSoa()
    {
    }
    public function updatePaymentSoaTagging($soa_no)
    {

        $soa = $this->db->query("SELECT  *  FROM  `soa_file` WHERE `soa_no` = '$soa_no'  LIMIT 1")->row();

        if (!empty($soa)) {
            $next_soa = $this->db->query("SELECT 
                    *
                FROM
                    soa_file
                WHERE
                    tenant_id = '$soa->tenant_id'
                        AND posting_date <= '$soa->posting_date'
                        AND soa_no <> '$soa->soa_no'
                ORDER BY posting_date DESC , soa_no DESC
                LIMIT 1
            ")->row();


            if (!empty($next_soa)) {
                $this->db->update('payment', ['soa_no' => $next_soa->soa_no], "soa_no = '$soa_no'");
            }
        }
    }
    ...
    public function getAllz($tbl_name, $orderBy)
    {
        $this->db->select('*');
        $this->db->from($tbl_name);
    ...
    public function getAll2($tbl_name)
    {
        // $query = $this->db->get($tbl_name);
        // return $query->result_array();
    ...
    public function get_deniedLprospect()
    {
        $query = $this->db->query(
            ...
            AND
                `p`.`prepared_by` = `u`.`id`
            AND
                (`p`.`status` =  'Denied' OR `p`.`status` =  'Revoked')
            AND
                `s`.`id` = `p`.`store_id`
            AND
                `p`.`flag` = 'Long Term'
    ...
    public function get_forwarded_balance($tenant_id, $startDate)
    {
        $query = $this->db->query("SELECT  inv.id,
                                            inv.document_type,
                                            (CASE 
    ...
    public function get_ledgerAdjustment($tenant_id, $searchType, $searchBy)
    {
        if ($searchBy == 'Show All') {
            $query = $this->get_ledgerAdjustmentAll($tenant_id);
        } else if ($searchBy == 'Doc No') {
            $query = $this->get_ledgerAdjustmentDocumentType($tenant_id, $searchType);
        } else if ($searchBy == 'Ref No') {
            $query = $this->get_ledgerAdjustmentReferenceNo($tenant_id, $searchType);
        } else {
            $query = $this->get_ledgerAdjustmentPostingDate($tenant_id, $searchType);
        }

        return $query;
    }

    public function get_ledgerAdjustmentAll($tenant_id)
    {
        $query = $this->db->query(" SELECT  
                                            inv.id,
                                            inv.document_type,
                                            (CASE
                                                WHEN 
                                                inv.gl_accountID = '4' THEN 'Basic Rent' ELSE 'Other Charges'
                                            END) AS type,
                                            inv.ref_no,
                                            inv.doc_no,
                                            inv.posting_date,
                                    ...
                                    LEFT JOIN
                                            ...
                                            (SELECT tenant_id,
                                                ...
                                                FROM subsidiary_ledger
                                                WHERE gl_accountID IN (22,29,4)
                                                AND document_type IN ('Invoice Adjustment', 'Debit Memo', 'Credit Memo')
                                                AND tenant_id <> 'DELETED'
                                                GROUP BY ref_no, gl_accountID, tenant_id) AS memo
                                    ON
                                            inv.ref_no = memo.ref_no AND inv.gl_accountID = memo.gl_accountID  AND inv.gl_accountID = memo.gl_accountID
                                    ...
                                    LEFT JOIN
                                            ...
                                            GROUP BY ref_no, gl_accountID, tenant_id) AS pmt
                                    ON 
                                            inv.ref_no = pmt.ref_no AND inv.gl_accountID = pmt.gl_accountID 
                                            GROUP BY inv.ref_no, inv.gl_accountID, inv.tenant_id
                                            ORDER BY inv.posting_date, inv.id, inv.gl_accountID");
    ...
    public function get_ledgerAdjustmentDocumentType($tenant_id, $doc_no)
    {
        $query = $this->db->query(" SELECT  
                                            inv.id,
                                            inv.document_type,
    ...
    public function get_ledgerAdjustmentReferenceNo($tenant_id, $ref_no)
    {
        $query = $this->db->query(" SELECT  
                                            inv.id,
                                            inv.document_type,
    ...
    public function get_ledgerAdjustmentPostingDate($tenant_id, $posting_date)
    {
        $query = $this->db->query(" SELECT  
                                            inv.id,
                                            inv.document_type,
    ...
    public function get_ledgerAdjustmentPayment($tenant_id, $ORNUMBER)
    {
        /*$query = $this->db->query("
            SELECT  
                (CASE
                    WHEN c.doc_no IS NOT NULL THEN 'Payment'
                    WHEN p.gl_accountID IN (4,22,29) AND i.document_type IS NOT NULL THEN i.document_type
                    WHEN p.gl_accountID = 9 THEN 'Sec. Deposit'
                    WHEN p.gl_accountID = 8 THEN 'Construction Bond'
                    WHEN p.gl_accountID = 7 THEN 'Advance'
                    ELSE ''
                END) as applied_to,
                (CASE
                    WHEN c.doc_no IS NOT NULL THEN c.doc_no
                    WHEN i.doc_no IS NOT NULL THEN i.doc_no
                    ELSE NULL
                END)  as doc_applied,
                a.gl_account,
                p.tenant_id,
                ...
                p.gl_accountID,
                (CASE
                    WHEN c.doc_no IS NOT NULL THEN SUM(c.amount  + IFNULL(p.debit,0) + IFNULL(p.credit,0) + IFNULL(adj.amount,0))
                    WHEN SUM(i.balance) IS NOT NULL THEN SUM(i.balance)
                    ELSE NULL
                END) as balance,
                SUM(IFNULL(adj.amount,0)) as pmt_adj,
                SUM(IFNULL(p.debit,0) + IFNULL(p.credit,0)) + SUM(IFNULL(adj.amount,0)) as pmt_amount,
                ABS(IFNULL(p.credit, 0)) as org_amount,
                 (CASE 
                        WHEN p.doc_no LIKE 'B%' 
                        THEN (SELECT p2.ref_no 
                            FROM subsidiary_ledger p2 
                            WHERE IFNULL(p2.debit, 0) > 0 
                            AND ABS(p2.debit) = ABS(p.credit)  
                            AND p2.doc_no = p.doc_no 
                            AND IFNULL(p2.ft_ref, '') = IFNULL(p.ft_ref, '') 
                            AND p2.posting_date = p.posting_date
                            AND p2.document_type = 'Payment'
            ...
            LEFT JOIN 
                ...
                WHERE document_type = 'Payment Adjustment'
                AND tenant_id = '$tenant_id'
                GROUP BY ref_no, gl_accountID, tenant_id, ft_ref
                ) 
                AS  adj
                ON p.ref_no = adj.ref_no 
                AND p.doc_no = adj.doc_no 
                AND IFNULL(p.ft_ref, '') = IFNULL(adj.ft_ref, '') 
                AND p.gl_accountID = adj.gl_accountID 
                AND p.tenant_id = adj.tenant_id
            LEFT JOIN
                (SELECT
                    doc_no,
                    ref_no,
                    ...
                    ft_ref,
                    status,
                    ABS(SUM(IFNULL(debit, 0) + IFNULL(credit, 0))) amount
                FROM 
                    subsidiary_ledger
                WHERE
                    document_type IN ('Payment', 'Payment Adjustment')
                AND
                    tenant_id = '$tenant_id'
                GROUP BY 
                    ref_no, gl_accountID, tenant_id, IFNULL(ft_ref, ''), doc_no
                ) AS c
                ON p.tenant_id = c.tenant_id 
                AND p.doc_no <> c.doc_no 
                AND p.ref_no = c.ref_no
                AND IFNULL(p.ft_ref, '') <> ''
                AND IFNULL(c.ft_ref, '') <> ''
                AND p.ft_ref = c.ft_ref
                AND p.gl_accountID = c.gl_accountID
                AND IFNULL(p.status, '') = ''
            LEFT JOIN
                (SELECT  
                    (CASE 
                        WHEN inv.gl_accountID = '4' AND inv.tag = 'Retro Rent' THEN 'Retro Rent'
                        WHEN inv.gl_accountID = '4' THEN 'Basic Rent'
                        WHEN inv.gl_accountID IN (22,29) AND inv.tag = 'Penalty' THEN 'Penalty'
                        ELSE 'Others'
                    END) AS document_type,
                    inv.doc_no,
                    inv.posting_date,
                ...
                LEFT JOIN
                    ...
                    FROM subsidiary_ledger p
                    WHERE p.gl_accountID IN (22,29,4)
                    AND p.document_type IN ('Payment', 'Payment Adjustment')
                    AND p.tenant_id = '$tenant_id'
                    GROUP BY p.ref_no, p.gl_accountID, p.tenant_id
                    ) AS pmt
                    ON  inv.ref_no = pmt.ref_no AND 
                        inv.gl_accountID = pmt.gl_accountID  AND 
                        inv.tenant_id = pmt.tenant_id
                ...
                ORDER BY 
                    document_type ASC, inv.gl_accountID ASC, inv.posting_date ASC) 
                AS i
                ON  p.ref_no = i.ref_no 
                    AND p.gl_accountID = i.gl_accountID 
                    AND p.tenant_id = i.tenant_id 
                    AND c.doc_no IS NULL
            LEFT JOIN
                gl_accounts a
                ON p.gl_accountID = a.id
            WHERE IFNULL(p.credit, 0) <> 0
            AND p.document_type = 'Payment'
            AND p.tenant_id = '$tenant_id'
            AND p.doc_no = '$ORNUMBER'
            GROUP BY p.ref_no, p.gl_accountID, p.tenant_id, p.id
            ");*/
        ...
        $query = $this->db->query("
            SELECT  
                ...
                p.gl_accountID,
                (CASE
                    WHEN c.doc_no IS NOT NULL THEN c.amount  + IFNULL(p.pmt_amount,0)
                    WHEN i.balance IS NOT NULL THEN i.balance
                    ELSE NULL
                END) as balance,
                p.debit_refno,
                p.org_amount,
            ...
            FROM (
                SELECT  
                    p1.*,
                    (CASE 
                        WHEN p1.doc_no LIKE 'B0%' OR p1.doc_no LIKE 'P0%'
                        THEN (SELECT p2.ref_no 
                            FROM subsidiary_ledger p2 
                            WHERE IFNULL(p2.debit, 0) > 0 
                            AND ABS(p2.debit) = ABS(p1.credit)  
                            AND p2.doc_no = p1.doc_no 
                            AND IFNULL(p2.ft_ref, '') = IFNULL(p1.ft_ref, '') 
                            AND p2.posting_date = p1.posting_date
                            AND p2.document_type = p1.document_type
                            AND p2.tenant_id = p1.tenant_id
                            LIMIT 1)
                        ELSE p1.ref_no
                    END) AS debit_refno,
                    ABS(IFNULL(p1.credit, 0)) as org_amount,
                    SUM((IFNULL(p1.debit, 0) + IFNULL(p1.credit, 0)) - IFNULL(p1.credit, 0)) as pmt_adj,
                    SUM(IFNULL(p1.debit, 0) + IFNULL(p1.credit, 0)) as  pmt_amount
                FROM subsidiary_ledger p1
                WHERE p1.document_type IN ('Payment', 'Payment Adjustment')
                AND p1.tenant_id = '$tenant_id'
                AND p1.doc_no = '$ORNUMBER'
                GROUP BY p1.doc_no, p1.ref_no, p1.gl_accountID, p1.tenant_id, p1.status, IFNULL(p1.ft_ref, '')
                HAVING pmt_amount <= 0 AND org_amount > 0
            ) p
            LEFT JOIN
                (SELECT
                    doc_no,
    ...
    public function get_adjNo($useNext = true)
    {
        $sequence = getSequenceNo(
            [
                'code' => "ADJ",
    ...
    public function get_otherCharges1()
    {
        $query = $this->db->query(
            "SELECT DISTINCT
                    `c`.`id`,
                ...
                FROM
                    `charges_setup` `c`
                WHERE
                    `c`.`charges_type` != 'Pre Operation Charges' ORDER BY `c`.`description`
        "
        );
    ...
    public function get_LUStorename($username)
    {
        $query = $this->db->query("
                ...
                ON
                        `leasing_users`.`user_group` = `stores`.`id`
                WHERE
                        `leasing_users`.`username` = '$username'
                ORDER BY
                        `first_name`");

        $ret = $query->row();
        return $ret->store_name;
    }

    public function get_UserType($username, $password)
    {
        $query = $this->db->query("SELECT user_type
                                   FROM leasing_users
                                   WHERE username = ? 
                                   AND password = ?", array($username, $password));
        $ret = $query->row();
        return $ret ? $ret->user_type : null; // Check if a result exists
    }


    public function get_SLdata($doc_no, $gl_accountID)
    {
        $query = $this->db->query(" SELECT * FROM subsidiary_ledger WHERE doc_no = '$doc_no' AND gl_accountID = '$gl_accountID';");

        $ret = $query->row();
        return $ret;
    }

    public function get_SLdata1($doc_no)
    {
        $query = $this->db->query(" SELECT * FROM subsidiary_ledger WHERE doc_no = '$doc_no' AND (tag = 'Other' OR tag = 'Penalty');");

        $ret = $query->row();
        return $ret;
    }

    public function managers_keyID($username, $password, $store_name)
    {
        $query = $this->db->query(
            "SELECT
                `u`.`id`,
    ...
    public function get_paymentData($Dref_no, $doc_no)
    {
        $query = $this->db->query("SELECT * 
                                    FROM 
                                        subsidiary_ledger 
                                    ...
                                    AND document_type = 'Payment' LIMIT 1");
        return $query->result_array();
    }

    public function get_paymentData2($ref_no, $doc_no, $org_amount)
    {
        $query = $this->db->query("SELECT * 
                                    FROM 
                                        subsidiary_ledger 
                                    WHERE 
                                        doc_no = '$doc_no' 
                                    AND ref_no = '$ref_no' 
                                    AND IFNULL(credit, 0) <> 0 
                                    AND document_type = 'Payment' LIMIT 1");
        return $query->result_array();
    }

    public function get_adjustmentsData($tenant_id)
    {
        $query = $this->db->query(" SELECT
                                        adj_no,
                                        doc_no,
                                        ...
                                        doc_type,
                                        transaction_date,
                                        CONCAT(m.first_name, ' ', m.middle_name, ' ', m.last_name) as modified_by,
                                    ...
                                    LEFT JOIN 
                                        leasing_users m
                                    ON 
                                        adjustments.modified_by = m.id
                                    LEFT JOIN 
                                        leasing_users a
                                    ON 
                                        adjustments.managers_key = a.id
                                    WHERE 
                                        tenant_id = '$tenant_id' AND adj_document IS NOT NULL GROUP BY adj_no");

        return $query->result_array();
    }

    public function get_tenantName($prospect_id)
    {
        $query = $this->db->query("SELECT trade_name FROM prospect WHERE id = '$prospect_id'")->row()->trade_name;
        return $query;
    }

    public function get_pID($tenant_id)
    {
        $query = $this->db->query(
            "SELECT
                `prospect_id`
    ...
    public function preparedBy($prepared_by)
    {
        $query = $this->db->query(" SELECT CONCAT(leasing_users.first_name, ' ', leasing_users.middle_name, ' ', leasing_users.last_name) AS prepared_by
                                    FROM adjustments
                                    RIGHT JOIN leasing_users 
                                    ON adjustments.modified_by = leasing_users.id 
                                    WHERE adjustments.modified_by = '$prepared_by' GROUP BY prepared_by LIMIT 1");
        // return $query; ->row()->prepared_by

        $ret = $query->row();
        return $ret;
    }

    public function count_print_adj($document_name)
    {
        $query = $this->db->query("SELECT COUNT(document_name) AS print_no FROM adjustment_print_log WHERE document_name = '$document_name'")->row()->print_no;

        return $query;
    }

    public function get_adjustmentsData1($document_name)
    {
        $query = $this->db->query(" SELECT 
                                            adj_no, 
                                            tenant_id,
                                    ...
                                    FROM 
                                            adjustments 
                                    LEFT JOIN 
                                            leasing_users m ON adjustments.modified_by = m.id 
                                    LEFT JOIN 
                                            leasing_users a ON adjustments.managers_key = a.id 
                                    WHERE 
                                            adj_document = '$document_name' AND adj_document IS NOT NULL GROUP BY adj_no");

        $ret = $query->row();
        return $ret;
    }

    public function get_adjustmentsData2($adj_no)
    {
        $query = $this->db->query(" SELECT 
                                            adj_no, 
                                            tenant_id, 
                                    ...
                                    LEFT JOIN 
                                            leasing_users a ON adjustments.managers_key = a.id 
                                    WHERE 
                                            adj_no = '$adj_no' AND adj_document IS NULL GROUP BY adj_no");
    ...
    public function get_preop_balance($tenant_id, $gl_account)
    {
        switch ($gl_account) {
            case 'Security Deposit':
                $gl_account = 'Security Deposit Payable';
                break;
            default:
                $gl_account;
                break;
        ...
        $query = $this->db->query("SELECT a.gl_account, SUM(ifnull(sl.debit,0) + ifnull(sl.credit,0)) * -1  AS balance
                                   FROM subsidiary_ledger  sl LEFT JOIN gl_accounts a ON a.id = sl.gl_accountID
                                   WHERE sl.tenant_id=? AND a.gl_account = ? AND sl.document_type IN ('Payment', 'Payment Adjustment')", [$tenant_id, $gl_account])->row();

        return $query;
    }

    public function get_preops_with_balance($tenant_id, $gl_account)
    {
        ...
        $result = $this->db->query("
            SELECT 
                sl.*,
                a.gl_account,
                SUM(ifnull(sl.debit,0) + ifnull(sl.credit,0)) * -1  AS balance
            FROM 
                subsidiary_ledger  sl
            LEFT JOIN
                gl_accounts a
            ON 
                a.id = sl.gl_accountID
            WHERE 
                sl.tenant_id = ? 
            AND 
                a.gl_account = ?
            ...
            HAVING 
                balance > 0
            ORDER BY
                sl.posting_date ASC, id ASC
        ", [$tenant_id, $gl_account])->result_object();

        return $result;
    }

    public function getLastestSOA($tenant_id, $date_filter = null)
    {
        $date_filter = empty($date_filter) ? '' : "AND posting_date <= '$date_filter'";
    ...
    public function get_old_soa_invoices_with_balance($tenant_id, $date = null)
    {
        $result = $this->db->query(
            "SELECT
                inv2.*,
                SUM(IFNULL(pmt.debit, 0) + IFNULL(pmt.credit, 0)) as pmt_amount,
                inv2.inv_amount + SUM(IFNULL(pmt.debit, 0) + IFNULL(pmt.credit, 0)) as balance
            FROM 
                (SELECT 
                    i.soa_no,
                    inv.*,
                    SUM(IFNULL(inv.debit, 0) + IFNULL(inv.credit, 0)) as inv_amount
                FROM
                    (SELECT 
                        s.soa_no,
                        i.doc_no,
                    ...
                    LEFT JOIN
                        `invoicing` i
                    ON 
                        s.invoice_id = i.id AND s.tenant_id = i.tenant_id
                    WHERE
                        s.tenant_id = '$tenant_id'
                    AND 
                        f.posting_date <= '$date'
                    GROUP BY
                        i.doc_no) AS i
                LEFT JOIN
                    subsidiary_ledger inv
                ON
                    inv.doc_no = i.doc_no AND
                    inv.tenant_id = i.tenant_id AND
                    inv.document_type IN ('Invoice', 'Invoice Adjustment', 'Credit Memo', 'Debit Memo') AND
                    inv.gl_accountID IN (4, 22, 29)
                GROUP BY
                    inv.tenant_id, inv.ref_no, inv.gl_accountID
                ) AS inv2
            LEFT JOIN 
                subsidiary_ledger pmt
            ON
                inv2.tenant_id = pmt.tenant_id AND
                inv2.gl_accountID = pmt.gl_accountID AND
                pmt.document_type IN ('Payment', 'Payment Adjustment') AND
                inv2.ref_no = pmt.ref_no
            GROUP BY
                inv2.tenant_id, inv2.gl_accountID, inv2.ref_no
            HAVING
                ROUND(balance) >= 1
            ORDER BY
                inv2.posting_date DESC, inv2.id"
        )->result_object();
        ...
        /* ===  OPTION 2 QUERY
            ...
            FROM
                ...
                FROM
                    ...
                    WHERE
                        s.tenant_id = 'ACT-LT000065'
                    AND 
                        f.posting_date <= '2021-06-07'
                    GROUP BY 
                        i.doc_no , s.soa_no) AS i
                LEFT JOIN 
                    subsidiary_ledger inv 
                ON 
                    inv.doc_no = i.doc_no
                    AND inv.tenant_id = i.tenant_id
                    AND inv.document_type IN ('Invoice' , 'Invoice Adjustment', 'Credit Memo', 'Debit Memo')
                    AND inv.gl_accountID IN (4 , 22, 29)
                GROUP BY 
                    inv.tenant_id , inv.ref_no , inv.gl_accountID , i.soa_no) AS inv2
                LEFT JOIN
                    subsidiary_ledger pmt 
                ON 
                    inv2.tenant_id = pmt.tenant_id
                    AND inv2.gl_accountID = pmt.gl_accountID
                    AND pmt.document_type IN ('Payment' , 'Payment Adjustment')
                    AND inv2.ref_no = pmt.ref_no
            GROUP BY 
                inv2.tenant_id , inv2.gl_accountID , inv2.ref_no , inv2.soa_no
            HAVING 
                ROUND(balance) >= 1
            ORDER BY 
                inv2.posting_date DESC , inv2.id"*/
    ...
    public function soa_no_exist_in_soa_line($soa_no, $tenant_id)
    {
        $result = $this->db->query("
            SELECT
                *
    ...
    public function prepost_exist_in_soa_line($soa_no, $doc_no)
    {
        $result = $this->db->query("
            SELECT
                *
    ...
    public function tenant_has_record_in_soa_line($tenant_id, $date = null)
    {
        ...
        $result = $this->db->query("
            ...
            AND 
                f.posting_date <= '$date'
            AND 
                l.soa_no IN (SELECT soa_no FROM soa_file WHERE tenant_id = '$tenant_id')
            ")->result_array();
        return count($result) > 0;
    }
    ...
    public function get_data_recon_sys_vs_nav_report($store, array $gl_ids, $date_from, $date_to)
    {
        ...
        $result = $this->db->query("
            ...
            (
                ...
                AND 
                    t.store_code = '$store'
                AND 
                    g.document_type IN ('Invoice', 'Invoice Adjustment', 'Payment', 'Payment Adjustment') 
                AND
                    (g.tenant_id <> 'DELETED' AND g.ref_no <> 'DELETED' AND g.doc_no <> 'DELETED')
                AND 
                    g.tenant_id <> 'ICM-LT000064'
                ...
                GROUP BY
                    ...
                    THEN  CONCAT(TRIM(g.doc_no),TRIM(g.ref_no),TRIM(g.gl_accountID),TRIM(g.tenant_id))
                    ELSE CONCAT(TRIM(g.gl_accountID), TRIM(g.posting_date), TRIM(g.doc_no),TRIM(g.tenant_id), TRIM(g.ref_no)) 
                    END
                ORDER BY 
                    g.document_type, g.doc_no
            ) 
    ...
    public function get_data_recon_sys_vs_bank_report($bank, $date_from, $date_to)
    {
        $result = $this->db->query("
            SELECT
                g.id,
                ...
                SUM(IFNULL(g.debit, 0)) debit,
                SUM(IFNULL(g.credit, 0)) credit,
                 SUM(IFNULL(g.debit, 0) + IFNULL(g.credit, 0)) amount
            FROM
                subsidiary_ledger as g
            LEFT JOIN
                (SELECT distinct(tnt.prospect_id) as prospect_id, tnt.tenant_id, tnt.store_code  FROM tenants tnt) t on t.tenant_id = g.tenant_id
            ...
            AND
                g.gl_accountID = '3'
            AND
                t.store_code LIKE '$bank->store_code%'
            AND
                g.bank_code = '$bank->bank_code'
            AND
            ...
            AND
                g.tenant_id <> 'ICM-LT000064'
            GROUP BY
                CONCAT(TRIM(g.gl_accountID), TRIM(g.posting_date), TRIM(g.doc_no),TRIM(g.tenant_id))
            ORDER BY
                g.document_type, g.posting_date, g.doc_no
        ")->result_object();

        return $result;
    }

    public function get_tmp_preop_invoice_with_balance($tenant_id, $date = null)
    {
        ...
        $result = $this->db->query("
            SELECT 
                p.*
            FROM
                tmp_preoperationcharges p
            LEFT JOIN 
                soa_line s
            ON 
                s.tenant_id = p.tenant_id AND
                s.preop_id = p.id
            WHERE
                p.tenant_id = ? AND 
                ROUND(p.amount) >= 1 AND
                s.id IS NULL AND
                p.posting_date <= ?
        ", [$tenant_id, $date])->result_object();

        return $result;
    }

    public function get_adj_for_soa_display($tenant_id, $doc_no)
    {
        $result = $this->db->query("
            SELECT 
                *,
                (SUM(IFNULL(debit, 0) + IFNULL(credit, 0)) * -1) as amount 
            FROM 
                adjustments
            WHERE
                tenant_id = ? AND 
                doc_no = ? AND
                gl_accountID NOT IN (22,29,4)
            GROUP BY gl_accountID
        ", [$tenant_id, $doc_no])->result_object();
    ...
    public function get_invoice_override_data($tenant_id)
    {
        $result = $this->db->query("
            SELECT 
                o.*,
                CONCAT(b.first_name, ' ', b.last_name) as override_by,
                CONCAT(m.first_name, ' ', m.last_name) as approved_by
            FROM
                invoice_override o
            LEFT JOIN
                leasing_users b
            ...
            WHERE 
                o.tenant_id = ?
        ", [$tenant_id])->result_object();

        foreach ($result as $key => $res) {
            $result[$key]->supp_docs = $this->get_invoice_override_docs(['inv_over_id' => $res->id]);
            $result[$key]->invoice_details = $this->get_invoice_details(['tenant_id' => $tenant_id, 'doc_no' => $res->doc_no]);
        }

        return $result;
    }

    public function get_invoice_override_docs(array $cond)
    {
        $result = $this->db
            ->select('*')
            ->from('invoice_override_docs')
            ->where($cond)
            ->get()
            ->result_object();
    ...
    public function get_invoice_details(array $cond)
    {
        $result = $this->db
            ->select('*')
            ->from('invoicing')
    ...
    public function getPaymentDetails($doc_no)
    {
        $result = $this->db->query("SELECT * FROM payment WHERE doc_no = '$doc_no' ORDER BY amount_paid DESC LIMIT 1");
        $ret = $result->row();
        return $ret;
    }

    public function getPaymetSchemeDetails($doc_no, $tenantID)
    {
        $result = $this->db->query("SELECT * FROM payment_scheme WHERE receipt_no = '{$doc_no}' AND tenant_id = '{$tenantID}' ORDER BY amount_paid DESC LIMIT 1");
        $ret = $result->row();
        return $ret;
    }

    public function getOnContracts($store, $type, $date1, $date2, $c_status)
    {
        $result = [];

        if ($c_status == 'Active Contracts') {
            if ($type == 'All Terms') {
                if ($store == 'All') {
                    $result = $this->db->query("SELECT
                                        ...
                                        t.tenancy_type,
                                        t.flag,
                                        (select count(file_name) from contract_docs where tenant_id = t.tenant_id) as has_docs
                                        FROM prospect p
                                        LEFT JOIN tenants t
                                        ON p.id = t.prospect_id
                                        LEFT JOIN contract_docs c
                                        ON t.tenant_id = c.tenant_id
                                        WHERE p.status = 'On Contract'
                                        AND t.status = 'Active'
                                        AND t.flag = 'Posted'
                                        AND opening_date BETWEEN '" . $date1 . "' AND '" . $date2 . "'
                                        GROUP BY t.tenant_id")->result_array();
                } else {
                    $result = $this->db->query("SELECT
                                            t.tenant_id,
                                            p.trade_name,
            ...
            if ($type == 'All Terms') {
                if ($store == 'All') {
                    $result = $this->db->query("SELECT
                                        ...
                                        LEFT JOIN contract_docs c
                                        ON t.tenant_id = c.tenant_id
                                        WHERE p.status IN ('On Contract', 'Approved')
                                        AND t.status = 'Active'
                                        AND t.flag = 'Pending'
                                        AND opening_date BETWEEN '" . $date1 . "' AND '" . $date2 . "'
                                        GROUP BY t.tenant_id")->result_array();
    ...
    public function getTenantID($prospectID)
    {
        $result = $this->db->query("SELECT * FROM tenants WHERE prospect_id ='$prospectID'")->row();
        return $result;
    }

    public function getLeasingDeno($date)
    {
        $result = $this->db->query("SELECT * FROM leasing_denomination WHERE date_from = '$date' AND inputted_by = '" . $this->session->userdata('id') . "'")->row();
        return $result;
    }

    public function leasing_denomination($date1, $date2)
    {
        $result = $this->db->query("SELECT * FROM leasing_denomination WHERE date_from = '$date1' AND date_to = '$date2' AND inputted_by = '" . $this->session->userdata('id') . "'")->result_array();
        return $result;
    }

    public function gettenantswht($tenancytype, $store)
    {
    ...
    public function getcontractdocuments($id)
    {
        $tenants = $this->db->query("SELECT * FROM tenants WHERE id = '" . $id . "' AND flag = 'Posted' AND status = 'Active'")->ROW();
        $documents = $this->db->query("SELECT distinct term FROM contract_docs WHERE tenant_id = '" . $tenants->tenant_id . "' ORDER BY term ASC")->RESULT_ARRAY();

        return $documents;
    }

    public function get_tenant_details_2($tenant_id)
    {
        return $query = $this->db->query("SELECT * FROM tenants 
                                          WHERE tenant_id = '" . $tenant_id . "' 
                                          AND flag IN ('Posted', 'Pending') 
                                          AND status IN ('Active', 'Terminated')
                                          ORDER BY id DESC")->ROW();
    }

    public function generate_paymentSlipNo($useNext = TRUE)
    {
        $sequence = getSequenceNo(
            [
                'code' => "PS",
                ...
                'lpad' => '7',
                'pad_string' => '0',
                'description' => "Payment Slip No Temporary OR"
            ],
            [
                'table' => 'subsidiary_ledger',
                'column' => 'doc_no'
    ...
    public function getAdvanceTransactionNo($useNext = TRUE)
    {
        $sequence = getSequenceNo(
            [
                'code' => "PS",
    ...
    public function getReprintSoaLogNo($soano)
    {
        return $this->db->query("SELECT * FROM soa_reprint_log WHERE soa_no = '{$soano}'")->ROW();
    }

    public function getReprintPaymentLogNo($docno)
    {
        return $this->db->query("SELECT * FROM payment_reprint_log WHERE doc_no = '{$docno}'")->ROW();
    }

    public function getInvoice($casDocNo)
    {
        return $this->db->query("SELECT * FROM subsidiary_ledger WHERE ref_no = '$casDocNo' and tag IN ('Basic Rent', 'Other')")->ROW();
    }

    public function getPreopInvoice($casDocNo)
    {
        return $this->db->query("SELECT doc_no, cas_doc_no, posting_date, SUM(org_amount) as amount FROM tmp_preoperationcharges WHERE cas_doc_no = '$casDocNo'")->ROW();
    }

    public function getInvoice002($casDocNo)
    {
        $invoiceData = [];
        $ref_no = $this->db->query("SELECT distinct(ref_no) as ref_no FROM subsidiary_ledger WHERE cas_doc_no = '$casDocNo'")->RESULT_ARRAY();

        foreach ($ref_no as $key => $value) {
            $invoiceData[] = $this->db->query("SELECT * FROM subsidiary_ledger WHERE ref_no = '{$value['ref_no']}' AND tag IN ('Basic Rent', 'Other')")->ROW();
        }

        return $invoiceData;
    }

    ######################## FOR SAMPLE DATA PURPOSES ############################################
    public function generate_RRreports_test($month, $docs, $store)
    {
        // $store    = $this->session->userdata('store_code');
        $invoices = $this->db->query("
            SELECT 
            ...
            AND 
                doc_no LIKE '%$docs%'
            AND
               (document_type = 'Invoice' OR document_type = 'Invoice Adjustment')
            AND 
    ...
    public function generate_ARreports_test($month, $docs, $store)
    {
        // $store = $this->session->userdata('store_code');
        ...
        $invoices = $this->db->query("
            ...
            FROM
                subsidiary_ledger sl
            WHERE
                (sl.document_type = 'Invoice' OR sl.document_type = 'Invoice Adjustment')
            AND
    ...
    public function getBankAcroname($bankCode)
    {
        return $this->db->select('acroname')
            ->from('accredited_banks')
            ->where('bank_code', $bankCode)
            ->get()
            ->row();
    }

    public function getGlAccount($gl_accountID)
    {
        return $this->db->select('*')
            ->from('gl_accounts')
            ->where('id', $gl_accountID)
            ->limit(1)
            ->get()
            ->row();
    }

    // public function generateIFTReports($post_date, $tenantid){
    //     return $entries = $this->db->query("
    //         SELECT 
    //             tbl.*,
    //             SUM(tbl.db) AS debit,
    //             SUM(tbl.cr) AS credit,
    ...
    public function findInSubsidiaryLedger($doc_no)
    {
        $sl = $this->db->query("SELECT * FROM subsidiary_ledger WHERE doc_no = '" . $doc_no . "' AND gl_accountID IN ('4', '22', '29', '7', '8', '9')")->result_array();

        return $sl;
    }

    public function generate_paymentCollectionAdjustment($post_date, $tenantid)
    {
                                        ...
                                        FROM
                                            ...
                                            LEFT JOIN (SELECT DISTINCT
                                                (tnt.prospect_id) AS prospect_id,
                                                    ...
                                                    tnt.store_code
                                            FROM
                                                tenants tnt) t ON t.tenant_id = g.tenant_id
                                            LEFT JOIN prospect p ON p.id = t.prospect_id
                                            LEFT JOIN gl_accounts a ON a.id = g.gl_accountID
                                            WHERE
                                                DATE(g.posting_date) LIKE '%$post_date%'
                                                    ...
                                                    AND (g.export_batch_code IS NULL
                                                    OR g.export_batch_code = '')
                                            GROUP BY g.doc_no , g.ref_no , g.gl_accountID , g.tenant_id , g.posting_date
                                            HAVING amt <> 0
                                            ORDER BY g.document_type , g.doc_no) AS tbl
                                                LEFT JOIN
                                            (SELECT 
                                                s.*, s.gl_accountID AS partnerID
                                            FROM
                                            ...
                                            GROUP BY s.doc_no , s.ref_no , s.credit) sl2 ON tbl.doc_no = sl2.doc_no
                                                AND tbl.ref_no = sl2.ref_no
                                                AND tbl.db <> 0
                                                AND tbl.db = ABS(sl2.credit)
                                                AND tbl.document_type = sl2.document_type
                                        GROUP BY tbl.doc_no , tbl.gl_accountID , tbl.posting_date , tbl.document_type , sl2.partnerID
                                        ORDER BY tbl.posting_date , tbl.doc_no , tbl.id , credit DESC , debit DESC , tbl.trade_name , tbl.ref_no")->result_object();
    }

    public function checkOtherCharges($ref_no)
    {
        return $this->db->query("SELECT * FROM `agc-pms-cas-local`.subsidiary_ledger 
                                 WHERE ref_no = '$ref_no' 
                                 AND cas_doc_no LIKE '%OLS%' 
                                 AND gl_accountID IN ('22', '29') 
                                 AND document_type = 'Invoice'")->ROW();
    }

    public function generate_RRreports_internal($month)
    {
        $store = $this->session->userdata('store_code');
        $invoices = $this->db->query("SELECT doc_no, ref_no
                                      FROM subsidiary_ledger
                                      ...
                                      AND (tenant_id <> 'DELETED' AND ref_no <> 'DELETED' AND doc_no <> 'DELETED')
                                      AND tenant_id <> 'ICM-LT000064'
                                      AND tenant_id LIKE '%$store%'
                                      AND (document_type = 'Invoice' OR document_type = 'Invoice Adjustment')
                                      AND (export_batch_internal IS NULL OR export_batch_internal = '')
                                      GROUP BY doc_no")->result();
        $doc_nos = [];
        ...
        $data = $this->db->query("SELECT 
                                ...
                                AND g.tenant_id != 'ICM-LT000064'
                                AND t.store_code = '$store'
                                GROUP BY g.doc_no, g.ref_no, g.gl_accountID, g.tenant_id
                                ORDER BY g.document_type, g.doc_no) AS tbl
                            LEFT JOIN 
                                    (SELECT 
                                        memo.ref_no,
                            ...
                            ON (tbl.ref_no = m.ref_no AND tbl.gl_accountID = m.gl_accountID AND tbl.document_type = 'Invoice')
                            GROUP BY tbl.gl_accountID
                            ORDER BY tbl.debit DESC,  tbl.credit DESC, tbl.trade_name, tbl.gl_account, tbl.doc_no,  tbl.id, tbl.document_type")->result_array();
        return compact('data', 'doc_nos');
    }

    public function generate_ARreports_internal($month)
    {
        $store = $this->session->userdata('store_code');
        $invoices = $this->db->query("SELECT sl.doc_no
                                      FROM subsidiary_ledger sl
                                      ...
                                      AND (sl.gl_accountID = '12' OR sl.gl_accountID = '13' OR sl.gl_accountID = '14' OR sl.gl_accountID = '15' OR sl.gl_accountID = '16' OR sl.gl_accountID = '17' OR sl.gl_accountID = '18' OR sl.gl_accountID = '20' OR sl.gl_accountID = '22' OR sl.gl_accountID = '29' OR sl.gl_accountID = '30' OR ( sl.gl_accountID = '5' AND sl.tag = 'Expanded'))
                                      AND DATE_FORMAT(sl.posting_date, '%M %Y') = '$month'
                                      AND (sl.export_batch_internal IS NULL OR sl.export_batch_internal = '')
                                      AND (sl.tenant_id <> 'DELETED' AND sl.ref_no <> 'DELETED' AND sl.doc_no <> 'DELETED')
                                      AND sl.tenant_id IN (SELECT t.tenant_id FROM tenants t, prospect p where  t.store_code = '$store' AND t.tenant_id != 'ICM-LT000064' AND t.prospect_id = p.id group by tenant_id)
                                      GROUP BY sl.doc_no, sl.tenant_id")->result();
        $doc_nos = [];
        ...
        $data = $this->db->query("SELECT
                                ...
                                FROM
                                    (SELECT
                                        ...
                                        ga.gl_account,
                                            (CASE
                                                ...
                                                SUM(sl.credit) + (SELECT IFNULL(SUM(sl1.debit), 0) FROM subsidiary_ledger sl1 WHERE sl.ref_no = sl1.ref_no AND sl1.gl_accountID = sl.gl_accountID AND sl1.document_type = 'Credit Memo' AND sl1.tenant_id != 'DELETED')
                                            END) AS amount
                                    FROM subsidiary_ledger sl, gl_accounts ga
                                    WHERE sl.gl_accountID = ga.id
                                    AND (sl.gl_accountID = '12' OR sl.gl_accountID = '13' OR sl.gl_accountID = '14' OR sl.gl_accountID = '15' OR sl.gl_accountID = '16' OR sl.gl_accountID = '17' OR sl.gl_accountID = '18' OR sl.gl_accountID = '20' OR sl.gl_accountID = '22' OR sl.gl_accountID = '29' OR sl.gl_accountID = '30' OR ( sl.gl_accountID = '5' AND sl.tag = 'Expanded'))
                                    AND (sl.document_type = 'Invoice' OR sl.document_type = 'Invoice Adjustment')
                                    AND DATE_FORMAT(sl.posting_date, '%M %Y') = '$month'
                                    ...
                                    AND (sl.export_batch_internal IS NULL OR sl.export_batch_internal = '')
                                    AND sl.tenant_id IN (SELECT t.tenant_id FROM tenants t, prospect p where  t.store_code = '$store' AND t.tenant_id != 'ICM-LT000064' AND t.prospect_id = p.id group by tenant_id)
                                    GROUP BY sl.gl_accountID, sl.ref_no) gl
                                GROUP BY gl.gl_account
                                ORDER BY gl.amount DESC")->result_array();
    ...
    # PMS DATABASE NOT USE

    public function insert_consol($table, $data)
    {
        $this->consol->insert($table, $data);
    }

    public function insert_batch_consol($table, $data)
    {
        $this->consol->insert_batch($table, $data);
    }

    public function update_tmp_latepaymentpenalty_consol($id, $doc_no)
    {
        $this->consol->query("UPDATE `tmp_latepaymentpenalty` SET `flag` = 'Invoiced', `invoice_no` = '$doc_no' WHERE `id` = '$id'");
    }

    public function update_ledgerDueDate_consol($tenant_id, $doc_no, $new_due_date)
    {
        $this->consol->query("UPDATE `ledger` SET `due_date` = '$new_due_date' WHERE `doc_no` = '$doc_no' AND `tenant_id` = '$tenant_id' AND `flag` = 'Penalty'");
    }

    public function checkOrNumber($ORNUMBER)
    {
        $query = $this->db->query("SELECT * FROM payment_scheme WHERE or_no = '{$ORNUMBER}'")->ROW();
        return $query;
    }

    public function getSoaData($soa_no)
    {
    ...
    public function select_tradeName_1($data, $tenancy_type)
    {
        $type = "Short Term";
        if ($tenancy_type == 'Long Term Tenant') {
            $type = "Long Term";
    ...
    // public function get_soaReprintDetails($storeID, $soa_no)
    // {
    //     return $this->db->query(
    //         "SELECT 
    //         `s`.`store_name`,
    ...
    //         `location_code` `l`
    //         WHERE `s`.`id` ='$storeID '
    //         AND `sf`.`soa_no` = '$soa_no'
    //         AND `sl`.`soa_no` = '$soa_no'
    //         AND `sf`.`soa_no` = `sl`.`soa_no`
    //         AND `t`.`tenant_id` = `sf`.`tenant_id`
    //         AND `t`.`tenant_id` = `sl`.`tenant_id`
    //         AND `pr`.`id` = `t`.`prospect_id`
    //         AND `l`.`id` = `t`.`locationCode_id`
    //         AND `l`.`tenancy_type` = `t`.`tenancy_type`
    //         -- AND `pr`.`tenancy_type` = `l`.`tenancy_type`
    //         "
    //     )->ROW();
    // }

    // public function get_paymentDataHistory($receiptNo)
    // {
    //     return $this->db->query(
    //         "SELECT 
    //                             `ps`.`receipt_no`,
    ...
    // public function get_paymentDescription($receiptNo)
    // {
    //     $this->db->select('gl.*, su.*, so.amount, ps.amount_due, ps.amount_paid , iv.charges_type, iv.transaction_date');
    //     $this->db->from('general_ledger gl');
    //     $this->db->join('subsidiary_ledger su', 'gl.tenant_id = su.tenant_id AND  gl.ref_no = su.ref_no');
    //     $this->db->join('soa_line so', 'su.tenant_id = so.tenant_id AND su.doc_no = so.doc_no ');
    //     $this->db->join('payment_scheme ps', 'so.tenant_id = ps.tenant_id AND so.soa_no = ps.soa_no');
    //     $this->db->join('invoicing iv', 'su.tenant_id = iv.tenant_id AND so.doc_no = iv.doc_no');
    //     $this->db->where('gl.document_type', 'Payment');
    //     $this->db->where('su.document_type', 'Invoice');
    //     $this->db->where_in('iv.charges_type', ['Basic', 'Other']);
    //     $this->db->where('gl.doc_no', $receiptNo);
    //     $this->db->where('ps.receipt_no', $receiptNo);
    //     // $this->db->group_by('iv.doc_no'); // Group by transaction_date to avoid duplicates
    //     $query = $this->db->get();
