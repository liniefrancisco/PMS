...
// }
    ...
    //     //          AND gl1.document_type = 'Credit Memo' 
    ...
    //     // general_ledger gl
    //     // JOIN 
    //     //     (SELECT t.tenant_id, p.trade_name 
    //     //      FROM tenants t
    //     //      JOIN prospect p ON t.prospect_id = p.id 
    //     //      WHERE t.store_code = '" . $this->session->userdata('store_code') . "' 
    //     //      AND t.tenant_id != 'DELETED'
    //     //      GROUP BY t.tenant_id
    //     //     ) tenants
    //     // ON tenants.tenant_id = gl.tenant_id
    //     // WHERE 
    //     //     (gl.gl_accountID = '4' OR gl.gl_accountID = '22' OR gl.gl_accountID = '29')
    //     // AND 
    //     //     (gl.document_type = 'Invoice' OR gl.document_type = 'Invoice Adjustment')
    ...
    //     //     DATE_FORMAT(gl.posting_date, '%M %Y') = '$month'
    //     // GROUP BY
    //     //     gl.tenant_id, gl.gl_accountID , gl.ref_no
    //     // ORDER BY
    //     //     tenants.trade_name ASC"
    //     //     );
    //     // return $query->result_array();
    //     // $query = $this->db->query(
    //     //     "SELECT
    ...
    //             gl.tenant_id,
    //             tenants.trade_name,
    //             SUM(gl.debit) - IFNULL((SELECT SUM(ABS(gl1.credit)) FROM subsidiary_ledger gl1 WHERE gl.ref_no = gl1.ref_no AND gl1.adj_tag = 'Credit Memo' AND gl1.gl_accountID = gl.gl_accountID), 0)  AS amount,
    //             gl.gl_accountID
    //         FROM
    //             subsidiary_ledger gl,
    //             (SELECT t.tenant_id, p.trade_name FROM tenants t, prospect p WHERE t.store_code = '" . $this->session->userdata('store_code') . "' AND t.tenant_id != 'DELETED' AND t.prospect_id =  p.id GROUP BY t.tenant_id) AS tenants
    //         WHERE
    //             ( gl.gl_accountID = '4' ||  gl.gl_accountID = '22' )
    //         AND
    //             (gl.document_type = 'Invoice' OR gl.document_type = 'Invoice Adjustment')
    //         AND
    //             DATE_FORMAT(gl.posting_date, '%M %Y') = '$month'
    ...
    //             NOT EXISTS (
    //                 SELECT 1
    //                 FROM subsidiary_ledger gl2
    //                 WHERE gl2.ref_no = gl.ref_no
    //                 AND gl2.gl_accountID = gl.gl_accountID
    //                 AND (gl2.document_type = 'Payment' OR gl2.document_type = 'Payment Adjustment')
    //             )
    //         GROUP BY
    //             gl.ref_no
    //         ORDER BY 
            ...
            "SELECT
                ...
                FROM
                    (SELECT
                        gl.tenant_id,
                        tenants.trade_name,
                        SUM(gl.debit) - IFNULL((SELECT SUM(ABS(gl1.credit)) FROM subsidiary_ledger gl1 WHERE gl.ref_no = gl1.ref_no AND gl1.gl_accountID = gl.gl_accountID), 0)  AS amount
                    FROM
                        subsidiary_ledger gl,
                        (SELECT t.tenant_id, p.trade_name FROM tenants t, prospect p WHERE t.store_code = '" . $this->session->userdata('store_code') . "' AND t.tenant_id != 'DELETED' AND t.prospect_id =  p.id GROUP BY t.tenant_id) AS tenants
                    WHERE
    ...
    //     report.tenant_id,
    //     report.trade_name,
    //     SUM(report.basic) AS basic_amount,   -- Sum of amounts where gl_accountID = '4'
    //     SUM(report.others) AS others_amount, -- Sum of amounts where gl_accountID IN ('22', '29')
    //     SUM(report.basic + report.others) AS total_amount -- Total amount (Basic + Others)
    // FROM
    //     (SELECT
    //         gl.tenant_id,
    //         tenants.trade_name,
    //         //  Amount for gl_accountID = '4' (Basic)
    //         CASE 
    ...
    //         //  Amount for gl_accountID = '22' or '29' (Others)
    //         CASE 
    ...
    //     report.tenant_id
    // HAVING 
    //     SUM(report.basic + report.others) > 0 // Exclude tenants with zero total amount
    // ORDER BY 
...
// --------------------------------------------
    ...
    public function generate_RRreportsAdjustment($month, $tenantid)
    {
        ...
        $data = $this->db->query("SELECT g.id, 
                                        ...
                                        LEFT JOIN (SELECT distinct(tnt.prospect_id) as prospect_id, tnt.tenant_id, tnt.store_code FROM tenants tnt) t ON t.tenant_id = g.tenant_id 
                                        LEFT JOIN prospect p on p.id = t.prospect_id LEFT JOIN gl_accounts a ON a.id = g.gl_accountID 
                                        LEFT JOIN invoicing inv on inv.tenant_id = g.tenant_id
                                        WHERE g.doc_no IN ('$docs') 
                                        AND (g.gl_accountID = '6' OR g.gl_accountID = '4' OR g.gl_accountID = '11' OR (g.gl_accountID = '5' AND (g.tag IS NULL OR g.tag = ''))) 
        ...
        return $this->db->query("SELECT 
                                ...
                                LEFT JOIN
                                    (SELECT 
                                        ...
                                        AND (s.document_type = 'Payment' OR g.document_type = 'Payment Adjustment')
                                        GROUP BY 
    ...
    public function get_soaReprintDetails($storeID, $soa_no)
    {
        return $this->db->query(
            ...
            AND `i`.`tenant_id` = `sf`.`tenant_id`
            -- AND `sub`.`tenant_id` = `i`.`tenant_id`
            -- AND `sub`.`doc_no` = `i`.`posting_date`
    ...
    public function get_paymentDescription($receiptNo)
    {
        ...
        $this->db->order_by('gl.id', 'ASC'); // Add order by ASC
        $query = $this->db->get();
    ...
    public function get_soaDescription($soa_no, $collection_date)
    {
        $this->db->select('iv.*,so.amount, iv.due_date, iv.doc_no,iv.id,iv.posting_date, iv.description');
        $this->db->from('soa_line so');
        $this->db->join('invoicing iv', 'so.tenant_id = iv.tenant_id');
        $this->db->where('so.soa_no', $soa_no);
        $this->db->where_in('iv.charges_type', ['Basic', 'Basic/Monthly Rental']);
        $this->db->where('iv.due_date', $collection_date);
        $this->db->group_by('iv.due_date');
        $this->db->order_by('iv.id', 'ASC'); // Add order by ASC
        $query = $this->db->get();
        return $query->result_array();
